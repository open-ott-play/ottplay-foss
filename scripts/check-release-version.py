...
for name in paths:
    ...
    if path.suffix == ".json":
        ...
        pattern = r'versionName\s+"([^\"]+)"' if path.suffix == ".gradle" else r"MARKETING_VERSION\s*=\s*([^;]+);"
        values = [value.strip().strip('"') for value in re.findall(pattern, path.read_text())]
        if not values or any(value != version for value in values):
            raise SystemExit(f"{name}: mobile version values {values} must all match {version}")
        actual = version
    else:
