...
for (const directory of ['dist', 'src-tauri/frontend']) tree(path.join(root, directory));
for (const directory of ['stb', 'prov', 'js', 'stbPlayer']) tree(path.join(root, directory));
for (const html of ['index.html', 'dist/index.html', 'src-tauri/frontend/index.html']) {
        ...
        if (fs.statSync(source).isDirectory()) checkCopies(relative);
            ...
            for (const stage of ['dist', 'src-tauri/frontend']) {
                ...
                if (!fs.existsSync(target) || !fs.readFileSync(source).equals(fs.readFileSync(target))) {
                    failures.push(stage + '/' + relative + ': missing or stale staged script');
                }
            }
        }
    }
