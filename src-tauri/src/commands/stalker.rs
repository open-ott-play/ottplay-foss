...
//!
//! Classic MAG requests are generated by the managed Stalker provider using
//! the shared client. This transport forwards its MAC cookie and bearer token.
...
fn is_allowed_url(url: &str) -> bool {
    url.contains("/stalker_portal/api/")
        ...
        // Classic Mag / Ministra path shapes (shared-core client).
...
pub async fn stalker_portal_fetch(
    ...
    let client = Client::builder()
        // Older Stalker PHP portals look up Authorization with case-sensitive keys.
...
mod tests {
    ...
    fn allows_foss_jsonrpc_and_swop() {
        ...
        assert!(is_allowed_url(
            "https://p.example/stalker_portal/stream/1.m3u8"
        ));
        assert!(is_allowed_url("https://host.example/swop/a.php"));
    }

    #[test]
    fn allows_classic_mag_path_shapes() {
        assert!(is_allowed_url(
        ...
        assert!(is_allowed_url("http://portal/portal.php"));
    }
