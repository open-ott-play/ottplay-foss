...
import { keys, dispatchKey } from '../keyhandler';
import { getWidthK, getHeightK, formatTwoDigits, time2time, getThumbnail } from '../utils/helpers';
...
Object.defineProperty(window, 'listKeyHandler', {
  set: function(v: any) { listKeyHandlerFn = v; },
  get: function(): any { return listKeyHandlerFn; },
  configurable: true,
  enumerable: true,
});
Object.defineProperty(window, 'getListItem', {
  set: function(v: any) { getListItemFn = v; },
  get: function(): any { return getListItemFn; },
  configurable: true,
  enumerable: true,
});
Object.defineProperty(window, 'detailListAction', {
  set: function(v: any) { detailListActionFn = v; },
  get: function(): any { return detailListActionFn; },
  configurable: true,
  enumerable: true,
});
var itemWidth = 735;
...
if (typeof (window as any).editvar === 'undefined') (window as any).editvar = '';
if (typeof (window as any).editCaption === 'undefined') (window as any).editCaption = '';
var editPos = 0;
...
var _keysSymbol: any[] = [
  ...
  { s: '&rarr;', a: function () { if (editPos < (window as any).editvar.length) { editPos++; _changeEdit(); } } },
  { s: '_', a: function () { (window as any).editvar = (window as any).editvar.substr(0, editPos) + ' ' + (window as any).editvar.substr(editPos); editPos++; _changeEdit(); } },
  { s: '', a: function () { if (editPos) { (window as any).editvar = (window as any).editvar.substr(0, editPos - 1) + (window as any).editvar.substr(editPos); editPos--; _changeEdit(); } } },
  { s: '', a: function () { } },
  { s: 'Ok', a: function () { clearInterval(cursorInterval); (window as any).restoreCPD(); $('#listEdit').hide(); if (typeof (window as any).setEdit === 'function') (window as any).setEdit(); } },
];
var _keyUp = false;
var _keyE = false;
...
export function uiInit(): void {
  ...
  $('#dialogbox').on('show', function () {
    $(this).css({ top: 0, left: 0, width: 'auto', height: 'auto' }).css({
  ...
  $infoBar.click(function (e: any) {
    if (!e) e = event;
    e.stopPropagation();
    if (typeof (window as any).showChanelInfo === 'function') (window as any).showChanelInfo();
  });
  ...
  if (listInEl) {
    var onWheel = function (e: any): void {
      ...
      if (t < 0 && (window as any).selIndex > 0 && typeof (window as any).changeSelect === 'function') (window as any).changeSelect(-1);
      if (t > 0 && (window as any).selIndex < ((window as any).listArray || []).length - 1 && typeof (window as any).changeSelect === 'function') (window as any).changeSelect(1);
    };
  ...
  $progressDiv.click(function (e: any) {
    ...
    var w = window as any;
    if (!w.playType && (!w.chanels || !w.curList || !w.chanels[w.curList[w.primaryIndex]] || !w.chanels[w.curList[w.primaryIndex]].rec)) return;
    e.stopPropagation();
    var t = (e.clientX - $progressDiv.position().left) / $progressDiv.width();
    if (w.playType < 0) {
      var r = Math.max(Math.round(t * w.stbGetLen()), 0);
      var hr = Math.floor(r / 3600);
      var mn = Math.floor((r % 3600) / 60);
      var sc = r % 60;
      if (typeof w.showShift === 'function') w.showShift('>> ' + (hr ? hr + ':' : '') + _t2(mn) + ':' + _t2(sc) + ' <<');
      if (typeof w.stbSetPosTime === 'function') w.stbSetPosTime(r);
      return;
    }
    var r2 = Math.round(t * (w._prog100.time_to - w._prog100.time) + w._prog100.time);
    if (r2 < Date.now() / 1e3) {
      if (!w.playType) {
        if (typeof w.timeShift === 'function') w.timeShift(Math.round(Date.now() / 1e3 - r2));
        return;
      }
      if (typeof w.showShift === 'function') w.showShift('>> ' + pos2text(r2) + ' <<');
      if (typeof w.playArchive === 'function') w.playArchive(r2);
    } else {
      if (typeof w.showShift === 'function') w.showShift(w._(w.playType ? 'Live' : 'Restart stream'));
      if (typeof w.playChannel === 'function') w.playChannel(w.catIndex, w.primaryIndex);
    }
  });
  ...
  $progressDiv.mousemove(function (e: any) {
    ...
    if (tooltipEl) {
      ...
      tooltipEl.style.top = ($progressDiv.offset().top - $progressDiv.height()) + 'px';
      tooltipEl.style.left = (clientX - tooltipEl.offsetWidth / 2) + 'px';
    }
    var $tooltipSpan = $('span', tooltipEl);
    var frac = (clientX - $progressDiv.position().left) / $progressDiv.width();
    if (w.playType < 0) {
      ...
      var r2 = Math.round(frac * (w._prog100.time_to - w._prog100.time) + w._prog100.time);
...
export function infoBarHideT(): void {
  if (typeof (window as any).stbIsPlaying === 'function') {
    if ($('#buffering').is(':visible') || !(window as any).stbIsPlaying() || $('#step').is(':visible')) {
...
export function showChanelInfo(timeoutSec: number): void {
  ...
  if (timeoutSec === 1 && $infoBar.is(':visible') && !$('#descr').is(':visible')) {
  ...
  if (!$infoBar.is(':visible')) {
    ...
    if (w.sInfoSlide) {
      $infoBar.slideDown(400, function () { if (typeof w.scrollUpDescr === 'function') w.scrollUpDescr(); });
    } else {
      $infoBar.show(0, function () { if (typeof w.scrollUpDescr === 'function') w.scrollUpDescr(); });
    }
    if (timeoutSec !== 2) {
      infoTimeout = setTimeout(infoBarHideT, (timeoutSec > 0 ? timeoutSec : (w.sInfoTimeout || 5)) * 1000);
    }
  } else if (!$('#descr').is(':visible')) {
    if (w.sInfoSlide) {
      $('#descr').slideDown(400, function () { if (typeof w.scrollUpDescr === 'function') w.scrollUpDescr(); });
    } else {
      $('#descr').show(0, function () { if (typeof w.scrollUpDescr === 'function') w.scrollUpDescr(); });
    }
  } else {
    infoBarHide();
  }
...
export function showPage(): void {
  ...
  try {
    if (settings.noSmall) {
      ...
      $('#list_window').show();
      if (typeof (window as any).stbSetWindow === 'function') (window as any).stbSetWindow();
    }
  } catch (e) {
    console.error(e);
  }
  if (listElement) listElement.style.display = '';
  var dataArr = listDataArray.length ? listDataArray : ((window as any).listDataArray || []);
  var pageStart = Math.floor(selIndex / settings.pageSize) * settings.pageSize;
  var pageEnd = Math.min(pageStart + settings.pageSize, dataArr.length);
  var itemHeight = (window.innerHeight - 90 * getHeightK()) / settings.pageSize;
  var html = '';
  if (dataArr.length > settings.pageSize) {
    itemWidth = getWidthK() * 720;
    (window as any).itemWith = itemWidth;
    var scrollWidth = 10 * getWidthK();
    var totalPages = Math.floor(dataArr.length / settings.pageSize) + (dataArr.length % settings.pageSize ? 1 : 0);
    var currentPage = Math.floor(selIndex / settings.pageSize);
    html += '<div onclick="event.stopPropagation();changeSelect(' + settings.pageSize + ');" style="float:right;height:100%;width:' + scrollWidth + 'px; border: 1px solid #f0f0f0;">';
    html += '<div onclick="event.stopPropagation();changeSelect(-' + settings.pageSize + ');" style="width:100%;height:' + (currentPage / totalPages * 100) + '%;"></div>';
    html += '<div style="background-color: #888;width:100%;height:' + (100 / totalPages) + '%;"></div></div>';
  } else {
  ...
  for (var i = pageStart; i < pageEnd; i++) {
    ...
    html += '<div id="it' + i + '" onclick="event.stopPropagation();setSelect(' + i + ')" class="item"';
    html += ' style="height:' + itemHeight + 'px; line-height:' + itemHeight + 'px; width:' + itemWidth + 'px;';
    if (selected) html += 'color:' + (curColor || 'gold') + '; background-color:' + (curColorB || '#668') + ';';
    html += '">';
    try {
...
export function changeSelect(delta: number): void {
  ...
  else if (selIndex >= dataArr.length) selIndex = delta === 1 ? 0 : dataArr.length - 1;
...
export function closeList(): void {
  ...
  try {
    ...
    if (typeof (window as any).stbToFullScreen === 'function') (window as any).stbToFullScreen();
  } catch (e) {
    console.error(e);
  }
}
...
export function infoBox(message: string): void {
  $('#dialogbox').html(message + '<br/><br/>' + btnDiv(keys.ENTER, strENTER, 'Ok')).show();
 ...
 */
export function confirmBox(message: string, onYes: () => void, onNo?: () => void): void {
  $('#dialogbox').html(message + '<br/><br/>' + btnDiv(keys.ENTER, strENTER, 'Yes') + btnDiv(keys.RETURN, strRETURN, 'No')).show();
 ...
 */
export function showSelectBox(s: number, n: string[], i: (val: number) => void, a?: number): void {
  ...
  function r(e: number) {
    ...
    n.forEach(function (val, t) {
      html += '<div style="' + (t === s ? 'color:' + (window as any).curColor + ';background-color:' + (window as any).curColorB : '') + '" onclick="(window as any)._doKey(' + (-100 + t) + ');">&nbsp;&nbsp;' + val + '&nbsp;&nbsp;</div>';
    });
    if (numprogElement) numprogElement.innerHTML = html;
    if (a) (window as any).numTimeout = setTimeout(function () {
      if (numprogElement) numprogElement.style.display = 'none';
...
export function updateChanelInfo(channelId: number): void {
  ...
  var _ch = (window as any).chanels ? (window as any).chanels[channelId] : undefined;
  if (!_ch || !_ch.time_to || _ch.time_to < Date.now() / 1000) {
    getCurProgData(channelId, function () { setTimeout(function() { updateChanelInfo(channelId); }, 0); });
  }
  ...
  // Channel number
  if (channelNumEl) channelNumEl.innerHTML = '' + ((primaryIndex != null ? primaryIndex : -1) + 1);

  // Channel info from global chanels
  var t = (window as any).chanels ? (window as any).chanels[channelId] : undefined;
  if (t) {
    if (channelNameEl) channelNameEl.innerHTML = t.channel_name || '';
    if (piconEl) piconEl.style.backgroundImage = typeof (window as any).getChannelPicon === 'function'
      ? 'url("' + (window as any).getChannelPicon(channelId) + '")'
      : (t.logo ? 'url("' + t.logo + '")' : '');
  } else {
    if (channelNameEl) channelNameEl.innerHTML = (typeof _ === 'function' ? _('Channel is not available!!!') : 'N/A') + ' id=' + channelId;
  ...
  if (t && t.name && t.time && t.time_to) {
    ...
    if (endTimeEl) endTimeEl.textContent = '+' + (remainingMin > 0 ? remainingMin : 0);
    if (programDurationEl) {
      programDurationEl.innerHTML = time2str(t.time) + ' - ' + time2time(t.time_to)
        + ' (<span id="cur_time">' + (Math.round((nowSec - t.time) / 60)) + '/</span>'
        + Math.round((t.time_to - t.time) / 60) + ' ' + (typeof _ === 'function' ? _('min') : 'min') + ')';
    }
    if (programDescrEl) {
      programDescrEl.innerHTML = (typeof getThumbnail === 'function' ? getThumbnail(t.icon || t.logo) : '') + (t.descr || '');
    }
    // Next program
    if (t.nextpr && t.nextpr.length) {
      if (nprogramNameEl) nprogramNameEl.innerHTML = t.nextpr[0].name;
      if (nbeginTimeEl) nbeginTimeEl.textContent = time2time(t.nextpr[0].time);
      var nextDur = Math.round((t.nextpr[0].time_to - t.nextpr[0].time) / 60);
      if (nendTimeEl) nendTimeEl.textContent = '' + (nextDur > 0 ? nextDur : 0);
    }
  } else {
...
export function initBackgroundIntervals(): void {
  setInterval(function () {
    ...
    if (typeof (window as any).playType !== 'undefined' && (window as any).playType && typeof (window as any).stbIsPlaying === 'function' && (window as any).stbIsPlaying()) {
 ...
 */
export function btnDiv(keyLabel: number, label: string, description: string, num?: string, extra?: string): string {
  ...
  if ((window as any).sNoColorKeys && [keys.RED, keys.GREEN, keys.YELLOW, keys.BLUE].indexOf(keyLabel) !== -1) a = '';
  if (num) a += '<div class="btn">' + num + '</div>&nbsp;';
  if (extra) a += '<div class="btn">' + extra + '</div>&nbsp;';
  if (!a) description = '<div class="btn">' + description + '</div>';
  return '<span onclick="_doKey(' + keyLabel + ');">' + a + description + '</span>&nbsp;&nbsp;';
}
...
export function pos2text(e: number): string {
  ...
  return _t2(t.getHours()) + ':' + _t2(t.getMinutes()) + ':' + _t2(t.getSeconds());
}
...
export function time2str(e: number): string {
  ...
  return days[d.getDay()] + '&nbsp;' + _t2(d.getDate()) + '.' + _t2(d.getMonth() + 1) + '&nbsp;' + _t2(d.getHours()) + ':' + _t2(d.getMinutes());
}
...
export function scrollUpDescr(): void {
  ...
  $('#programm_descr').stop(true).css('margin-top', 0);
  var e = $('#programm_descr').height() - $('#descr').height() + $('#programm_name2').height() + $('#programm_duration').height();
...
export function infoProgramm(title: string): void {
  ...
  if (listPodvalElement) {
    ...
    listPodvalElement.innerHTML = btnDiv(keys.RETURN, strRETURN, 'Close') + (title ? btnDiv(keys.N2, strInfo, 'TMDb', extra) : '');
  }
  aboutKeyHandler = function (e: number): boolean {
    if (title) {
      switch (e) {
        ...
        case keys.N2:
        case keys.INFO:
          if ((window as any).TMDb && typeof (window as any).TMDb.search === 'function') (window as any).TMDb.search(title);
          return true;
      }
    }
    return false;
...
export function infoMedia(): void {
  ...
  if (listPodvalElement) {
    ...
    listPodvalElement.innerHTML = btnDiv(keys.RETURN, strRETURN, 'Close') + (t ? btnDiv(keys.N2, strInfo, 'TMDb', extra) : '');
  }
  aboutKeyHandler = function (e: number): boolean {
    if (t) {
      switch (e) {
        ...
        case keys.N2:
        case keys.INFO:
          if ((window as any).TMDb && typeof (window as any).TMDb.search === 'function') (window as any).TMDb.search(t);
          return true;
      }
    }
    return false;
...
export function infoList(e?: string): void {
  ...
  listDataArray = [];
  infoArr.forEach(function (item: any) { listDataArray.push(_(item.name || '')); });
  if (!(window as any).sNoNumbersKeys) {
  ...
  if (typeof e !== 'undefined') {
    for (var t = 0; t < infoArr.length; t++) {
      if (infoArr[t].action === e) { selIndex = t; break; }
    }
  }
  getListItemFn = function (item: any, _idx: number) { return '&nbsp;&nbsp;' + item; };
  ...
  listKeyHandlerFn = function (key: number): boolean {
    switch (key) {
      ...
      case keys.ENTER: {
        var arr = (window as any).infoArr || [];
        if (arr[selIndex] && arr[selIndex].action) arr[selIndex].action();
        return true;
      }
      case keys.N2:
      case keys.INFO:
        if (typeof (window as any).pluginInfo === 'function') (window as any).pluginInfo();
        return true;
    }
    return false;
  };
  if (listCaptionElement) listCaptionElement.innerHTML = _('Info');
  if (listPodvalElement) listPodvalElement.innerHTML = btnDiv(keys.RETURN, strRETURN, 'Close');
  showPage();
}
...
export function popupList(i?: any): void {
  ...
  function splitSlash(e: string, t: boolean): string {
    try { e = e.split('/')[t ? 1 : 0].trim(); } catch (ex) { console.error(ex); }
    return e;
  }
  ...
  var c: any = false;
  try { c = curList[primaryIndex]; } catch (e) { console.error(e); }
  ...
  popupActions.forEach(function(action: any, t: number) {
    ...
    try {
      switch (action) {
        case toggleAudioTrack:
          if (!c || typeof (window as any).stbAudioTracksExists !== 'function' || !(window as any).stbAudioTracksExists()) return;
          break;
        case toggleSubtitle:
          if (!c || typeof (window as any).stbSubtitleExists !== 'function' || !(window as any).stbSubtitleExists()) return;
          break;
        case popPause:
          r = splitSlash(r, !(window as any).stbIsPlaying ? false : (window as any).stbIsPlaying());
        case popShift:
        case popRecords:
          if (playType < 0 || !c || !chanels[c] || chanels[c].rec) break;
          else return;
        case popTogglePip:
        ...
        case popStop:
          ...
          break;
        case popMedia:
          if (typeof (window as any).getMediaArray !== 'function') return;
          break;
        case popPrevProg:
    ...
    if (!sNoNumbersKeys) {
      switch (action) {
        case toggleAudioTrack: n = '1'; break;
        case infoList: n = '2'; break;
        case popPrevProg: n = '3'; break;
        case popShift: n = '4'; break;
        case popTogglePip: n = '5'; break;
        case popStopPip: n = '6'; break;
        case popStop: n = '7'; break;
        case restart: n = '8'; break;
        case optionsList: n = '9'; break;
        case exitPortal: n = '0'; break;
      }
    ...
    if (!sNoColorKeys) {
      n = '';
      switch (action) {
        case popBuckets: n = 'blue'; break;
        case popEpg: n = 'red'; break;
        case popRecords: n = 'green'; break;
        case popMedia: n = 'yellow'; break;
      }
      if (n) r = '<div class="btn ' + n + '">&nbsp;</div> ' + r;
    }
    ...
    n = '';
    switch (action) {
      case infoList: n = strInfo || 'Info'; break;
      case popPrevProg: n = strPRECH || 'Prev'; break;
      case popTogglePip: n = strPip || 'PiP'; break;
      case toggleAudioTrack: n = strAudio || 'Audio'; break;
      case toggleSubtitle: n = strSubt || 'Subt'; break;
      case (window as any).toggleZoom: n = strZoom || 'Zoom'; break;
      case (window as any).toggleAspectRatio: n = strAspect || 'Aspect'; break;
      case optionsList: n = strTools || 'Tools'; break;
      case popPause: n = strPlayPause || 'Play'; break;
      case popStop: n = strSTOP || 'Stop'; break;
    }
    if (n) r = '<div class="btn">' + n + '</div> ' + r;
  ...
  listKeyHandlerFn = function(key: any): boolean {
    console.log('DBG popupList handler: key=' + key + ' selIndex=' + selIndex + ' listArray.len=' + listArray.length + ' item=' + (listArray[selIndex] ? 'exists' : 'null'));
    switch (typeof key === 'number' ? key : key.keyCode) {
      ...
      case keys.ENTER: {
        var item = listArray[selIndex];
        console.log('DBG popupList ENTER: item=' + (item ? 'exists' : 'null') + ' item.action=' + (item && item.action ? typeof item.action : 'undefined'));
        if (item && item.action) {
      ...
      case keys.ZOOM:
        if (typeof (window as any).toggleZoom === 'function') (window as any).toggleZoom();
        return true;
      case keys.ASPECT:
        if (typeof (window as any).toggleAspectRatio === 'function') (window as any).toggleAspectRatio();
        return true;
      case keys.N0:
      ...
      case keys.N1:
      case keys.AUDIO:
        if (typeof (window as any).toggleAudioTrack === 'function') (window as any).toggleAudioTrack();
        return true;
      case keys.N2:
        if (typeof (window as any).infoList === 'function') (window as any).infoList();
        return true;
      case keys.N3:
        if (typeof (window as any).popPrevProg === 'function') (window as any).popPrevProg();
        return true;
      case keys.N4:
        if (typeof (window as any).popShift === 'function') (window as any).popShift();
        return true;
      case keys.N5:
        if (typeof (window as any).popTogglePip === 'function') (window as any).popTogglePip();
        return true;
      case keys.N6:
        if (typeof (window as any).popStopPip === 'function') (window as any).popStopPip();
        return true;
      case keys.N7:
        if (typeof (window as any).popStop === 'function') (window as any).popStop();
        return true;
      case keys.N8:
      ...
      case keys.TOOLS:
        ...
        return true;
      case keys.SUBT:
        if (typeof (window as any).toggleSubtitle === 'function') (window as any).toggleSubtitle();
        return true;
      case keys.EPG:
      case keys.RED:
        if (typeof (window as any).epgList === 'function') (window as any).epgList((window as any).catIndex, (window as any).primaryIndex, false);
        return true;
      case keys.GREEN:
        if (typeof (window as any).recordsList === 'function') (window as any).recordsList((window as any).catIndex, (window as any).primaryIndex, false);
        return true;
      case keys.BLUE:
      case keys.PREV:
        if (typeof (window as any).bucketsList === 'function') (window as any).bucketsList((window as any).catIndex);
        return true;
    }
    return false;
  };
...
export function popMedia(): void {
  if (typeof (window as any).getMediaArray === 'function' && typeof (window as any).mediaList === 'function') (window as any).mediaList(null);
}
...
export function popPrevProg(): void {
  closeList();
  if (typeof (window as any).prevProg === 'function') (window as any).prevProg();
}
...
export function popShift(): void {
  closeList();
  if (typeof (window as any).shiftArchiveSelect === 'function') (window as any).shiftArchiveSelect(0);
}
...
export function popPause(): void {
  closeList();
  if (typeof (window as any)._doKey === 'function') (window as any)._doKey((window as any).keys.N0);
}
...
export function popStop(): void {
  closeList();
  if (typeof (window as any)._doKey === 'function') (window as any)._doKey((window as any).keys.STOP);
}
...
export function popStopPip(): void {
  ...
  if (typeof (window as any).stbStopPip === 'function') (window as any).stbStopPip();
  closeList();
}
...
export function changeVolume(delta: number): void {
  ...
  t = Math.max(t, 0);
  t = Math.min(t, 100);
  if (typeof (window as any).stbSetVolume === 'function') (window as any).stbSetVolume(t);
  _showVolume(t);
}
...
export function togglePip(): void {
  if ((window as any).pipIndex == null) {
    ...
    if (typeof (window as any).stbPlayPip === 'function') (window as any).stbPlayPip((window as any).getChannelUrl((window as any).curList[(window as any).pipIndex]));
  } else {
    if ((window as any).pipCatIndex === (window as any).catIndex && (window as any).pipIndex === (window as any).primaryIndex) return;
    var e = (window as any).pipIndex;
    (window as any).pipIndex = (window as any).primaryIndex;
    (window as any).pipCatIndex = (window as any).catIndex;
    if (typeof (window as any).stbPlayPip === 'function') (window as any).stbPlayPip((window as any).getChannelUrl((window as any).curList[e]));
  }
}
...
export function hsvToRgb(h: number, s: number, v: number): number[] {
  ...
  switch (a) {
    case 0: r = v; g2 = u; b = l; break;
    case 1: r = c; g2 = v; b = l; break;
    case 2: r = l; g2 = v; b = u; break;
    case 3: r = l; g2 = c; b = v; break;
    case 4: r = u; g2 = l; b = v; break;
    default: r = v; g2 = l; b = c; break;
  }
...
export function colorDialog(): void {
  ...
  if (listPodvalElement) listPodvalElement.innerHTML = btnDiv(keys.RETURN, strRETURN, 'Close') + btnDiv(keys.ENTER, strENTER, 'Set');
  if (listDetailElement) listDetailElement.innerHTML = '';
  $('#listAbout').html('<div style="font-size:larger;">' + _('Color') + ':<br/><br/>&nbsp;<span id="step" style="font-size: 150%;">&nbsp;1234567890&nbsp;<span style="background-color:' + (window as any).curColorB + '">&nbsp;1234567890&nbsp;</span></span>&nbsp;<br/>' + '<br><div class="btn" onclick="_doKey(keys.LEFT);">' + strLEFT + '</div>&nbsp;<div class="btn" onclick="_doKey(keys.RIGHT);">' + strRIGHT + '</div>&nbsp;' + _('Color') + '<br><div class="btn" onclick="_doKey(keys.UP);">' + strUP + '</div>&nbsp;<div class="btn" onclick="_doKey(keys.DOWN);">' + strDOWN + '</div>&nbsp;' + _('Saturation') + '<br>' + btnDiv(keys.YELLOW, '', 'Yellow') + '<br>' + btnDiv(keys.GREEN, '', 'Green') + '<br>' + btnDiv(keys.BLUE, '', 'Blue') + '</div>').show();
  aboutKeyHandler = function (e: number): boolean {
    switch (e) {
      case keys.UP: n = Math.min(n + 5, 100); break;
      case keys.DOWN: n = Math.max(n - 5, 0); break;
      case keys.RIGHT: s += 10; if (s > 360) s = 0; break;
      case keys.LEFT: s -= 10; if (s < 0) s = 360; break;
      case keys.YELLOW: s = 50; n = 85; break;
      case keys.GREEN: s = 90; n = 85; break;
      case keys.BLUE: s = 180; n = 85; break;
      case keys.ENTER:
    ...
    $('#step').css('color', 'rgb(' + rgb[0] + ',' + rgb[1] + ',' + rgb[2] + ')');
    return true;
  };
  var rgb0 = hsvToRgb(s, n, 100);
  $('#step').css('color', 'rgb(' + rgb0[0] + ',' + rgb0[1] + ',' + rgb0[2] + ')');
}
...
export function selColorDialog(): void {
  ...
  if (listPodvalElement) listPodvalElement.innerHTML = btnDiv(keys.RETURN, strRETURN, 'Close') + btnDiv(keys.ENTER, strENTER, 'Set');
  if (listDetailElement) listDetailElement.innerHTML = '';
  $('#listAbout').html('<div style="font-size:larger;">' + _('Color') + ':<br/><br/>&nbsp;<span id="step" style="font-size: 150%;background-color:' + (window as any).curColorB + '">&nbsp;1234567890&nbsp;</span>&nbsp;</div>').show();
  aboutKeyHandler = function (e: number): boolean {
    switch (e) {
      case keys.UP: n = Math.min(n + 5, 100); break;
      case keys.DOWN: n = Math.max(n - 5, 0); break;
    ...
    $('#step').css('background-color', 'rgb(' + rgb[0] + ',' + rgb[1] + ',' + rgb[2] + ')');
    return true;
  };
  var rgb0 = hsvToRgb(s, n, 50);
  $('#step').css('background-color', 'rgb(' + rgb0[0] + ',' + rgb0[1] + ',' + rgb0[2] + ')');
}
...
export function backColorDialog(): void {
  ...
  saveCPD();
  if (listCaptionElement) listCaptionElement.innerHTML = _('Background color');
  if (listPodvalElement) listPodvalElement.innerHTML = btnDiv(keys.RETURN, strRETURN, 'Close') + btnDiv(keys.ENTER, strENTER, 'Set');
  if (listDetailElement) listDetailElement.innerHTML = '';
  $('#listAbout').html('<div style="font-size:larger;">' + _('Color') + ':<br/><br/>&nbsp;<span id="step" style="font-size: 150%;background-color:' + (window as any).curColorB + '">&nbsp;1234567890&nbsp;</span>&nbsp;</div>').show();
  aboutKeyHandler = function (e: number): boolean {
...
export function joyMenu(): void {
  ...
  $('#dialogbox').html('<table style="font-size:inherit">' +
    '<tr><td></td>' + btn + btnDiv(keys.UP, strUP, '<br>Rewind<br>') + '</td><td></td></tr>' +
    '<tr>' + btn + btnDiv(keys.LEFT, strLEFT, '<br>Menu') + '</td>' + btn + btnDiv(keys.ENTER, strENTER, '<br>Pause<br>') + '</td>' + btn + btnDiv(keys.RIGHT, strRIGHT, '<br>Toggle<br>sound track') + '</td></tr>' +
    '<tr><td></td>' + btn + btnDiv(keys.DOWN, strDOWN, (window as any).playType ? '<br>Live' : '<br>Previous<br>channel') + '</td><td></td></tr>' +
    '</table>' + btnDiv(keys.RETURN, strRETURN, 'Close')).show();
  dialogBoxKeyHandler = function (e: number): void {
    $('#dialogbox').hide();
    switch (e) {
      case keys.ENTER:
        if (typeof (window as any)._doKey === 'function') (window as any)._doKey((window as any).keys.PLAY);
        return;
      case keys.UP:
        if (typeof (window as any).shiftArchiveSelect === 'function') (window as any).shiftArchiveSelect(0);
        return;
      case keys.DOWN:
        (window as any).playType ? (typeof (window as any)._doKey === 'function' && (window as any)._doKey((window as any).keys.STOP)) : (typeof (window as any).prevProg === 'function' && (window as any).prevProg());
        return;
      case keys.RIGHT:
        if (typeof (window as any).toggleAudioTrack === 'function') (window as any).toggleAudioTrack();
        return;
...
function _setCase(e: boolean): void {
  ...
  if (!(window as any).sNoColorKeys) _keysSymbol[0].s = '<span style="border-bottom:3px solid red;">' + _keysSymbol[0].s + '</span>';
}
...
export function showEditKey1(_initKeys: any): void {
  saveCPD();
  if (typeof (window as any).stbGetItem === 'function' && (window as any).stbGetItem('ottplaylang') === '_eng') _keyE = true;
  _keysSymbol[1].s = typeof (window as any).stbGetItem === 'function' && (window as any).stbGetItem('ottplaylang') === '_eng' ? '' : '<span style="font-family:fontello;padding:0.2em;">&#xe80E;</span>';
  _keysSymbol[7].s = '<span style="font-family:fontello;padding:0.2em;">&#xe804;</span>';
  _keysSymbol[9].s = 'Ok';
  if (!(window as any).sNoColorKeys) {
    if (_keysSymbol[1].s) _keysSymbol[1].s = '<span style="border-bottom:3px solid green;">' + _keysSymbol[1].s + '</span>';
    if (_keysSymbol[7].s) _keysSymbol[7].s = '<span style="border-bottom:3px solid #bb0;">' + _keysSymbol[7].s + '</span>';
    if (_keysSymbol[9].s) _keysSymbol[9].s = '<span style="border-bottom:3px solid blue;">' + _keysSymbol[9].s + '</span>';
  }
...
export function showEdit(): void {
  ...
  r += '<div id="ee" style="width:100%;white-space:pre-wrap;word-wrap:break-word;"></div>';
  ...
  e.html(r).show();
  _changeEdit();
  $('#ik' + _keyCur).css({ 'background-color': (window as any).curColorB, 'color': (window as any).curColor });
  if (listPodvalElement)
    listPodvalElement.innerHTML = btnDiv(keys.RETURN, strRETURN, 'Close') + btnDiv(keys.RED, '', _keysSymbol[0] ? _keyUp ? '&darr;a' : '&uarr;A' : '', strTools) + btnDiv(keys.GREEN, '', _keysSymbol[1] ? _keyE ? _('lang') : 'English' : '', strFF) + btnDiv(keys.YELLOW, '', 'Delete', strRW) + btnDiv(keys.BLUE, '', 'Ok', strPlayPause);
}
...
export function _changeEdit(): void {
  $('#ee').html(
    (window as any).editvar.substr(0, editPos) + '<div id="cursor" style="display:inline-block;vertical-align:top;background-color:' + (window as any).curColor + ';width:3px;height:1.2em;"></div>' + (window as any).editvar.substr(editPos));
  clearInterval(cursorInterval);
  var blink = true;
  var cursor = $('#cursor');
  cursorInterval = setInterval(function () {
    blink = !blink;
    cursor.css('background-color', blink ? (window as any).curColor : 'inherit');
  }, 500);
}
...
export function clickKey(e: number): void {
  if (typeof (window as any).event !== 'undefined' && (window as any).event && (window as any).event.stopPropagation) (window as any).event.stopPropagation();
...
export function editKey1(e: number): void {
  ...
  if (ch && _keys.indexOf(ch) !== -1) {
    ...
    if (idx === _keyCur) {
      ...
      if (_keys.charCodeAt(idx) > 9) {
        (window as any).editvar = (window as any).editvar.substr(0, editPos) + _keys[idx] + (window as any).editvar.substr(editPos);
        editPos++;
        _changeEdit();
      } else {
        if (_keysSymbol[_keys.charCodeAt(idx)] && typeof _keysSymbol[_keys.charCodeAt(idx)].a === 'function')
      ...
      _keyCur = idx;
      $('#ik' + _keyCur).css({ 'background-color': (window as any).curColorB, 'color': (window as any).curColor });
      (window as any).editvar = (window as any).editvar.substr(0, editPos) + _keys[_keyCur] + (window as any).editvar.substr(editPos);
      editPos++;
      _changeEdit();
    }
    return;
  ...
  switch (e) {
    ...
    case (window as any).keys.RED:
      if (_keysSymbol[0] && typeof _keysSymbol[0].a === 'function') _keysSymbol[0].a();
      return;
    case (window as any).keys.FF:
    case (window as any).keys.GREEN:
      if (_keysSymbol[1] && typeof _keysSymbol[1].a === 'function') _keysSymbol[1].a();
      return;
    case (window as any).keys.RW:
    case (window as any).keys.YELLOW:
      if (_keysSymbol[7] && typeof _keysSymbol[7].a === 'function') _keysSymbol[7].a();
      return;
    case (window as any).keys.PLAY:
    case (window as any).keys.PAUSE:
    case (window as any).keys.BLUE:
      if (_keysSymbol[9] && typeof _keysSymbol[9].a === 'function') _keysSymbol[9].a();
      return;
    case (window as any).keys.ENTER:
      if (_keys.charCodeAt(_keyCur) > 9) {
        ...
        if (_keysSymbol[_keys.charCodeAt(_keyCur)] && typeof _keysSymbol[_keys.charCodeAt(_keyCur)].a === 'function')
    ...
    case (window as any).keys.RETURN:
      clearInterval(cursorInterval);
      if (typeof (window as any).restoreCPD === 'function') (window as any).restoreCPD();
      $('#listEdit').hide();
      return;
    default:
...
export function editKey2(code: number): void {
  switch (code) {
    case (window as any).keys.ENTER:
      (window as any).editvar = ($('#(window as any).editvar').val() as string) || '';
      if (typeof (window as any).setEdit === 'function') (window as any).setEdit();
      break;
    case (window as any).keys.EXIT:
    case (window as any).keys.RETURN:
      $('#listEdit').hide();
...
export function showEditKey2(_initKeys?: number[]): void {
  if (typeof (window as any).saveCPD === 'function') (window as any).saveCPD();
  ...
  if ((window as any).listCaptionElement) (window as any).listCaptionElement.innerHTML = caption;
  var html = caption + ':<br/><br/>';
  html += '<br/><input type="text" id="editvar" value="' + val.replace(/"/g, '&quot;') + '" style="background-color: black; color:' + ((window as any).curColor || '#fff') + '; font-size:150%; width: 95%;" autofocus><br/><br/>';
  html += '<br/>' + ((window as any).btnDiv || function() { return ''; })(keys.EXIT || 27, strExit, '- return without save');
  html += '<br/>' + ((window as any).btnDiv || function() { return ''; })(keys.ENTER || 13, strEnter, '- save');
...
export function mediaList(e: any): void {
  ...
  if (typeof e === 'string') {
    if (e === 'submenu') {
      ...
      var r = mediaRecords[selIndex].title || mediaRecords[selIndex].playlist_name || undefined;
  ...
  if (typeof (window as any).getMediaArray === 'function') (window as any).getMediaArray(e, showMediaList);
}
...
export function clickVal(e: number): void {
  if (typeof (window as any).event !== 'undefined' && (window as any).event && (window as any).event.stopPropagation) (window as any).event.stopPropagation();
  if (_curVal === e && aboutKeyHandler) aboutKeyHandler((window as any).keys.ENTER);
  $('#ik' + _curVal).css({ 'background-color': '', 'color': '' });
  _curVal = e;
  $('#ik' + _curVal).css({ 'background-color': (window as any).curColorB, 'color': (window as any).curColor });
}
...
export function selectValue(t: any): void {
  var r = t.values.filter(function (v: any) { return v !== '@@@'; });
  ...
  if (maxW > 0 && listAboutW > 0) {
    n = Math.max(Math.min(Math.round(listAboutW / maxW) - 1, r.length), Math.round(r.length / 6) + 1);
  }
  ...
  for (var i = 0; i < r.length; i++) {
    if (i % n === 0) html += '<br/>';
    html += '<div id="ik' + i + '" onclick="clickVal(' + i + ');" style="display:inline-block;width:' + (98 / n) + '%;overflow:hidden;text-align:center;vertical-align:middle;line-height:' + lineHeight + 'px;">' + r[i] + '</div>';
  }
  $('#listAbout').html('<div style="font-size:larger;">' + html + '</div>').show();
  $('#ik' + _curVal).css({ 'background-color': (window as any).curColorB, 'color': (window as any).curColor });
  ...
  aboutKeyHandler = function (e: number): boolean {
    switch (e) {
      case keys.UP:
        move(_curVal >= n ? -n : r.length - r.length % n + (_curVal + 1 > r.length % n ? -n : 0));
        return true;
      case keys.DOWN:
        move(_curVal < r.length - n ? n : -_curVal + _curVal % n);
        return true;
      case keys.LEFT:
        move(_curVal % n > 0 ? -1 : _curVal + n - 1 > r.length - 1 ? r.length - _curVal - 1 : n - 1);
        return true;
      case keys.RIGHT:
        move(_curVal % n < n - 1 ? _curVal + 1 == r.length ? -_curVal % n : 1 : -n + 1);
        return true;
      case keys.ENTER:
...
export function exitPortal(): void {
  confirmBox(_('Do you want to exit player?'), function () {
    var w = window as any;
    if (typeof w.setCurrent === 'function') w.setCurrent(w.catIndex, w.primaryIndex);
    w.playType = 0;
