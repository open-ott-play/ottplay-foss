...
import {
  toggleAspectRatio, toggleZoom, toggleAudioTrack, toggleSubtitle, setPlayerMode,
} from '../core';
import {
  popPrevProg, popPause, popStop, popShift,
  popTogglePip, popStopPip, popBuckets, popEpg, popRecords, popMedia,
  exitPortal, infoList,
} from '../ui';
...
export var arrayProvaiders = [
  'm3u', 'stalker', 'xtream', '', 'ottclub', 'edem', 'shura', 'itv',
  'tvteam', 'ottg', 'great', 'top', 'shara.club', 'shara-tv', 'bestlist',
  'bestlist/stalker', 'all4you', 'ipstream', 'korona', 'antifriz',
  'kb-team', 'fox', 'iptv-ott.ru', 'dosug', 'topiptv', '1ott',
  'newlook', 'polmedia', 'dragon', 'only4', 'ottprime', 'shocktv',
  'diamondtv', 'fabryka', 'russkoetv', 'ultifl1x', 'tvclub', 'vidok',
  'cbilling', '', 'drvao', 'd/maxtv', 'moidom', 'sharavoz', 'raduga',
  'prost', 'fxml', 'rd', 'tabox',
];
...
var pdsa: string[] = ["catsArray", "cats", "favoritesArray", "parentalArray", "catIndex", "primaryIndex", "prevArr", "epgTimers", "aAspects", "aZooms", "aAudios", "aSubs", "sSortAbc", "sPlayers", "medHistory", "medFavorites"];
...
export var popupActions: any[] = [
  ...
  popPrevProg, popPause, popStop, popShift,
  popTogglePip, popStopPip, popBuckets, popEpg, popRecords, popMedia,
  noProvParam, nofun, optionsList, restart, exitPortal, infoList,
];
...
declare var getEPGchanel: (chId: string, cb: (id: string, data: any[]) => void) => void;
declare var getEPGchanelCached: (chId: string, cb: (id: string, data: any[]) => void) => void;
declare var epgCash: number;
declare var getCurProgData: (chId: string, cb: (chId: string) => void) => boolean;
...
function updateChanelList(chId: string): void {
  ...
  $('#pr' + chId).css('width', ((Date.now() / 1e3 - chanels[chId].time) / (chanels[chId].time_to - chanels[chId].time)) * 100 + '%');
...
function detailProg(): void {
  ...
  if (e.time_to && e.time_to >= Date.now() / 1e3) {
    ...
    var r = '<div id="_name"><div style="color:' + curColor + ';">' + e.name + '</div><div style="font-size:smaller;">' + time2time(e.time) + ' - ' + time2time(e.time_to) + ' (' + (t > 0 ? t + '/' : '') + Math.round((e.time_to - e.time) / 60) + ' ' + _('min') + ')</div></div>' + '<div id="_descr" style="font-size:smaller;overflow:hidden;"><div id="_prd">' + getThumbnail(e.icon) + e.descr + '</div></div>';
    if (e.nextpr && sNextCountL) {
      r += '<div id="_nextpr" style="' + (sShowDescr ? 'position:absolute;left:0;bottom:0;padding:4px;' : '') + 'width:98%;white-space:nowrap;font-size:smaller;">';
      e.nextpr.forEach(function(n: any, i: number) {
        if (i < sNextCountL) r += time2time(n.time) + ' <span style="color:' + curColor + ';">' + n.name + '</span></br>';
      });
      r += '</div>';
    }
    listDetail.innerHTML = r;
    var s = sShowDescr ? ($('#listDetail').height() - $('#_name').height() - ($('#_nextpr').height() || 0)) : 0;
    $('#_descr').height(s);
    s = $('#_prd').height() + 10 - s;
    scrollUp('_prd', s, 5000);
  }
  if (sPreview == 1) {
    if (typeof (window as any).previewChId === 'function') (window as any).previewChId(listArray[selIndex]);
  }
}
/**
...
function setPopupChannels(): void {
  if ((!sFavorites && listCatIndex) || (sFavorites && !listCatIndex)) {
    $('#listPopUp').html(btnDiv(keys.N1, '1', 'Move channel up') + '<br/>' + btnDiv(keys.N7, '7', 'Move channel down') + '<br/>' + btnDiv(keys.N8, '8', 'Delete channel') + (sFavorites ? '' : '<br/>' + btnDiv(keys.N3, '3', 'Add channel to category')));
  } else {
    $('#listPopUp').html(btnDiv(keys.N3, '3', 'Add channel to ' + (sFavorites ? 'favorites' : 'category')) + '<br/>' + btnDiv(keys.N9, '9', _('Sort channels') + ': ' + _(sSortAbc ? '"As Is"' : 'By alphabet')));
  }
...
declare var btnDiv: (key: number, label: string, desc: string, num?: string, extra?: string) => string;
...
declare var getScriptDOM: (url: string, onSuccess: () => void, onError: (e?: any) => void) => void;
/**
  ...
  try {
    if (typeof code === 'string') {
      ...
      if (parts.length >= 2) {
        ...
        if (key === 'd' || key === 'data') providerSetItem('dealerData', val);
      }
    }
  } catch (r) { console.error('[dealer]', r); }
}
...
export function optionsList(fn?: () => void): void {
  ...
  listDataArray = [];
  optionsArr.forEach(function(opt: any) { listDataArray.push(_(opt.name || '')); });
  ...
  if (typeof fn !== 'undefined') {
    for (var t = 0; t < optionsArr.length; t++) {
      if (optionsArr[t].action == fn) { selIndex = t; break; }
    }
  }
  ...
  listKeyHandlerFn = function(key: number): boolean {
    console.log('DBG optionsList handler: key=' + key + ' keys.ENTER=' + keys.ENTER + ' selIndex=' + selIndex + ' action=' + (optionsArr[selIndex] ? (typeof optionsArr[selIndex].action) : 'undefined'));
    switch (key) {
      case keys.RETURN:
        popupList(-1);
        return true;
      case keys.ENTER:
        console.log('DBG optionsList ENTER: optionsArr[selIndex].action=' + (optionsArr[selIndex].action ? 'function' : 'undefined'));
...
var nselprov = 0, nprovparams = 0, _clearAll = 0;
...
export function noProvParam(): void {
  ...
  confirmBox(e ? 'Show provider settings?' : 'Hide provider settings?', function() {
...
declare var getEPGchanelCur: ((chId: string, cb: (id: string, data: any[]) => void) => void) | null;
declare var getMediaArray: ((url: string, cb: (data: any[]) => void) => void) | null;
...
declare var confirmBox: (message: string, onYes: () => void, onNo?: () => void) => void;
...
export function loadProv(): void {
  ...
  function onError(): void {
    ...
    $(launch_id).append('<br/><b>Failed to load provider script !!!</b>').hide();
    firstRun();
  }
  ...
  if (!$('#launch').is(':visible')) {
    ...
    $('#dialogbox')
      .html('<img src="' + host + '/stbPlayer/buffering.gif?' + __av + '" height="40">')
      .show();
    launch_id = '#dialogbox';
    closeList();
  }
  ...
  if (typeof _providerGetItem !== 'undefined') providerGetItem = _providerGetItem;
  if (typeof _providerHasItem !== 'undefined') providerHasItem = _providerHasItem;
  if (typeof _providerHasItemValue !== 'undefined') providerHasItemValue = _providerHasItemValue;
  if (typeof _providerSetItem !== 'undefined') providerSetItem = _providerSetItem;
  if (typeof _providerDelItem !== 'undefined') providerDelItem = _providerDelItem;
  ...
  getScriptDOM(
    ...
    function () {
      try {
        ...
        console.log('[loadProv] Script loaded, checking duneAddSettings');
        if (typeof duneAddSettings === 'function') {
          ...
          console.log('[loadProv] popupActions.length:', popupActions.length, 'noProvParam:', (window as any).noProvParam);
          console.log('[loadProv] popupActions.indexOf(noProvParam):', popupActions.indexOf(noProvParam));
          var idx = popupActions.indexOf(noProvParam) + 1;
          console.log('[loadProv] idx after +1:', idx);
          console.log('[loadProv] BEFORE duneAddSettings - popupArray.length:', popupArray.length, 'popupActions.length:', popupActions.length);
          console.log('[loadProv] popupArray before:', popupArray.slice());
          duneAddSettings(idx);
          console.log('[loadProv] AFTER duneAddSettings - popupArray.length:', popupArray.length);
          console.log('[loadProv] popupArray after:', popupArray.slice());
          console.log('[loadProv] duneAddSettings completed, popupArray:', popupArray.slice(0, 5), '...');
          ...
          console.log('[loadProv] optionsList index:', popupActions.indexOf(optionsList));
          console.log('[loadProv] idx:', idx);
          console.log('[loadProv] popupActions.length before splice check:', popupActions.length);
          console.log('[loadProv] popupArray before window update:', popupArray.slice());
          (window as any).popupActions = popupActions;
          (window as any).popupArray = popupArray;
          (window as any).popupDetail = popupDetail;
          console.log('[loadProv] window.popupArray length after update:', (window as any).popupArray.length);
          console.log('[loadProv] window.popupArray full:', (window as any).popupArray.slice());
          (window as any).popupActions = popupActions;
          (window as any).popupArray = popupArray;
          (window as any).popupDetail = popupDetail;
          if (parseInt(stbGetItem('noProvParam') || '0')) {
            var count = popupActions.indexOf(optionsList) - idx;
            console.log('[loadProv] noProvParam=1, about to splice count:', count, 'items from idx:', idx);
            console.log('[loadProv] popupArray before splice:', popupArray.slice());
            popupArray.splice(idx, count);
            popupDetail.splice(idx, count);
            popupActions.splice(idx, count);
            console.log('[loadProv] popupArray after splice:', popupArray.slice());
          }
          if (
...
export function loadChannels(): void {
  if (!$('#launch').is(':visible')) {
    ...
    if (launch_id !== '#dialogbox')
      $('#dialogbox')
        .html('<center><img src="' + host + '/stbPlayer/buffering.gif?' + __av + '" height="40">')
        .show();
    launch_id = '#dialogbox';
    closeList();
  }
  ...
  for (var _ck in chanels) { delete chanels[_ck]; }
  epg = {};
  epgCashObj = {};
  epgCashArr = [];
  curList = [];
...
function _channelsList(catIdx: number, channelIdx: number): void {
  ...
  getListItemFn = function (chId: string, idx: number) {
    ...
    var textW =
      ...
      2 * progMargin -
      archWidth * 3;
    var progName = getCurProgData(chId, updateChanelList)
      ? ch.name
      : '';
    if (ch.outdated === true)
      progName =
