...
var _tvclub_cfg = { server: '', user: '', pass: '', m3u: '' };
function _tvclub_load() {
    try { var d = providerGetItem('cfg'); if (d) _tvclub_cfg = JSON.parse(d); } catch(e) {}
    if (!_tvclub_cfg.server && !_tvclub_cfg.m3u) _tvclub_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _tvclub_save() { providerSetItem('cfg', JSON.stringify(_tvclub_cfg)); }
...
function getChanelsArray(cb) {
    _tvclub_load();
    if (_tvclub_cfg.server && _tvclub_cfg.user && _tvclub_cfg.pass) _tvclub_xtream(cb);
    else if (_tvclub_cfg.m3u) _tvclub_m3u(cb);
    else { alert(_('Configure TVClub in Settings -> Provider Settings')); cb(); }
}
function _tvclub_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _tvclub_cfg.m3u, timeout: 15e3,
        success: function(d) { _tvclub_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _tvclub_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _tvclub_parseM3U(d, cb); },
...
function _tvclub_xtream(cb) {
    ...
    var api = _tvclub_cfg.server + '/player_api.php?username=' + encodeURIComponent(_tvclub_cfg.user) + '&password=' + encodeURIComponent(_tvclub_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _tvclub_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _tvclub_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _tvclub_cfg.server + '/live/' + encodeURIComponent(_tvclub_cfg.user) + '/' + encodeURIComponent(_tvclub_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _tvclub_cfg.m3u = _tvclub_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_tvclub_cfg.user) + '&password=' + encodeURIComponent(_tvclub_cfg.pass) + '&type=m3u_plus&output=ts';
        _tvclub_m3u(cb);
    });
}
function duneAddSettings(e) {
    _tvclub_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('TVClub settings'));
    popupActions.splice(e, 1, _tvclub_edit);
    var idx = popupActions.indexOf(_tvclub_edit);
    if (idx > -1) {
        var lbl = _('TVClub settings');
        if (_tvclub_cfg.server && _tvclub_cfg.user) lbl += ': ' + _tvclub_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _tvclub_cfg.user + ')';
        else if (_tvclub_cfg.m3u) lbl += ': ' + _tvclub_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _tvclub_edit() {
    selIndex = 0; _tvclub_load();
    var srv = _tvclub_cfg.server, usr = _tvclub_cfg.user, pwd = _tvclub_cfg.pass, m3u = _tvclub_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _tvclub_cfg.server = srv; _tvclub_cfg.user = usr; _tvclub_cfg.pass = pwd; _tvclub_cfg.m3u = m3u; _tvclub_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('TVClub');
