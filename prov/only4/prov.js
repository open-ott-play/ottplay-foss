...
var _only4_cfg = { server: '', user: '', pass: '', m3u: '' };
function _only4_load() {
    try { var d = providerGetItem('cfg'); if (d) _only4_cfg = JSON.parse(d); } catch(e) {}
    if (!_only4_cfg.server && !_only4_cfg.m3u) _only4_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _only4_save() { providerSetItem('cfg', JSON.stringify(_only4_cfg)); }
...
function getChanelsArray(cb) {
    _only4_load();
    if (_only4_cfg.server && _only4_cfg.user && _only4_cfg.pass) _only4_xtream(cb);
    else if (_only4_cfg.m3u) _only4_m3u(cb);
    else { alert(_('Configure Only4.tv in Settings -> Provider Settings')); cb(); }
}
function _only4_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _only4_cfg.m3u, timeout: 15e3,
        success: function(d) { _only4_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _only4_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _only4_parseM3U(d, cb); },
...
function _only4_parseM3U(data, cb) {
...
function _only4_xtream(cb) {
    $(launch_id).append(_('Loading from API...'));
    var api = _only4_cfg.server + '/player_api.php?username=' + encodeURIComponent(_only4_cfg.user) + '&password=' + encodeURIComponent(_only4_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _only4_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _only4_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _only4_cfg.server + '/live/' + encodeURIComponent(_only4_cfg.user) + '/' + encodeURIComponent(_only4_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _only4_cfg.m3u = _only4_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_only4_cfg.user) + '&password=' + encodeURIComponent(_only4_cfg.pass) + '&type=m3u_plus&output=ts';
        _only4_m3u(cb);
    });
}
function duneAddSettings(e) {
    _only4_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('Only4.tv settings'));
    popupActions.splice(e, 1, _only4_edit);
    var idx = popupActions.indexOf(_only4_edit);
    if (idx > -1) {
        var lbl = _('Only4.tv settings');
        if (_only4_cfg.server && _only4_cfg.user) lbl += ': ' + _only4_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _only4_cfg.user + ')';
        else if (_only4_cfg.m3u) lbl += ': ' + _only4_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _only4_edit() {
    selIndex = 0; _only4_load();
    var srv = _only4_cfg.server, usr = _only4_cfg.user, pwd = _only4_cfg.pass, m3u = _only4_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _only4_cfg.server = srv; _only4_cfg.user = usr; _only4_cfg.pass = pwd; _only4_cfg.m3u = m3u; _only4_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('Only4.tv');
