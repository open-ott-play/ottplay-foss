...
var _fabryka_cfg = { m3u: "", pass: "", server: "", user: "" };
function _fabryka_load() {
    try {
        var d = providerGetItem("cfg");
        if (d) _fabryka_cfg = JSON.parse(d);
    } catch (e) {}
    if (!(_fabryka_cfg.server || _fabryka_cfg.m3u))
        _fabryka_cfg = { m3u: "", pass: "", server: "", user: "" };
}
function _fabryka_save() {
    providerSetItem("cfg", JSON.stringify(_fabryka_cfg));
}
...
function getChanelsArray(cb) {
    _fabryka_load();
    if (_fabryka_cfg.server && _fabryka_cfg.user && _fabryka_cfg.pass)
        _fabryka_xtream(cb);
    else if (_fabryka_cfg.m3u) _fabryka_m3u(cb);
    else {
        alert(_("Configure Fabryka.TV in Settings -> Provider Settings"));
        cb();
    }
}
...
function _fabryka_xtream(cb) {
    ...
    var api =
        ...
        encodeURIComponent(_fabryka_cfg.user) +
        "&password=" +
        encodeURIComponent(_fabryka_cfg.pass);
...
function duneAddSettings(e) {
    ...
    popupDetail.splice(e, 1, _("Fabryka.TV settings"));
    popupActions.splice(e, 1, _fabryka_edit);
    var idx = popupActions.indexOf(_fabryka_edit);
    if (idx > -1) {
        var lbl = _("Fabryka.TV settings");
        if (_fabryka_cfg.server && _fabryka_cfg.user)
            lbl +=
                ": " +
                _fabryka_cfg.server.replace(/^https?:\/\//, "").split("/")[0] +
                " (" +
                _fabryka_cfg.user +
                ")";
        else if (_fabryka_cfg.m3u)
            lbl += ": " + _fabryka_cfg.m3u.substr(0, 40) + "...";
        popupArray[idx] = lbl;
    }
}
function _fabryka_edit() {
    selIndex = 0;
    _fabryka_load();
    var srv = _fabryka_cfg.server,
        usr = _fabryka_cfg.user,
        pwd = _fabryka_cfg.pass,
        m3u = _fabryka_cfg.m3u;
    function bl() {
        listArray = [
            _("Server") + ": " + (srv || ""),
            _("Login") + ": " + (usr || ""),
            ...
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 3:
                        ...
                        return true;
                    case 5:
                        _fabryka_cfg.server = srv;
                        _fabryka_cfg.user = usr;
                        _fabryka_cfg.pass = pwd;
                        _fabryka_cfg.m3u = m3u;
                        _fabryka_save();
                        duneAddSettings(0);
                        loadChannels();
                        return true;
    ...
    listCaption.innerHTML = _("Fabryka.TV");
