...
var _fabryka_cfg = { server: '', user: '', pass: '', m3u: '' };
function _fabryka_load() {
    try { var d = providerGetItem('cfg'); if (d) _fabryka_cfg = JSON.parse(d); } catch(e) {}
    if (!_fabryka_cfg.server && !_fabryka_cfg.m3u) _fabryka_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _fabryka_save() { providerSetItem('cfg', JSON.stringify(_fabryka_cfg)); }
...
function getChanelsArray(cb) {
    _fabryka_load();
    if (_fabryka_cfg.server && _fabryka_cfg.user && _fabryka_cfg.pass) _fabryka_xtream(cb);
    else if (_fabryka_cfg.m3u) _fabryka_m3u(cb);
    else { alert(_('Configure Fabryka.TV in Settings -> Provider Settings')); cb(); }
}
function _fabryka_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _fabryka_cfg.m3u, timeout: 15e3,
        success: function(d) { _fabryka_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _fabryka_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _fabryka_parseM3U(d, cb); },
...
function _fabryka_parseM3U(data, cb) {
...
function _fabryka_xtream(cb) {
    $(launch_id).append(_('Loading from API...'));
    var api = _fabryka_cfg.server + '/player_api.php?username=' + encodeURIComponent(_fabryka_cfg.user) + '&password=' + encodeURIComponent(_fabryka_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _fabryka_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _fabryka_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _fabryka_cfg.server + '/live/' + encodeURIComponent(_fabryka_cfg.user) + '/' + encodeURIComponent(_fabryka_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _fabryka_cfg.m3u = _fabryka_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_fabryka_cfg.user) + '&password=' + encodeURIComponent(_fabryka_cfg.pass) + '&type=m3u_plus&output=ts';
        _fabryka_m3u(cb);
    });
}
function duneAddSettings(e) {
    _fabryka_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('Fabryka.TV settings'));
    popupActions.splice(e, 1, _fabryka_edit);
    var idx = popupActions.indexOf(_fabryka_edit);
    if (idx > -1) {
        var lbl = _('Fabryka.TV settings');
        if (_fabryka_cfg.server && _fabryka_cfg.user) lbl += ': ' + _fabryka_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _fabryka_cfg.user + ')';
        else if (_fabryka_cfg.m3u) lbl += ': ' + _fabryka_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _fabryka_edit() {
    selIndex = 0; _fabryka_load();
    var srv = _fabryka_cfg.server, usr = _fabryka_cfg.user, pwd = _fabryka_cfg.pass, m3u = _fabryka_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _fabryka_cfg.server = srv; _fabryka_cfg.user = usr; _fabryka_cfg.pass = pwd; _fabryka_cfg.m3u = m3u; _fabryka_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('Fabryka.TV');
