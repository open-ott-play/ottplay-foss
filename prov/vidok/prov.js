...
function _vidok_load() {
    try {
        ...
        if (d) _vidok_cfg = JSON.parse(d);
    } catch (e) {}
    if (!(_vidok_cfg.server || _vidok_cfg.m3u))
        _vidok_cfg = { server: "", user: "", pass: "", m3u: "" };
}
function _vidok_save() {
    providerSetItem("cfg", JSON.stringify(_vidok_cfg));
}
...
function getChanelsArray(cb) {
    _vidok_load();
    if (_vidok_cfg.server && _vidok_cfg.user && _vidok_cfg.pass)
        _vidok_xtream(cb);
    else if (_vidok_cfg.m3u) _vidok_m3u(cb);
    else {
        alert(_("Configure Vidok.TV in Settings -> Provider Settings"));
        cb();
    }
}
function _vidok_m3u(cb) {
    ...
    $.ajax({
        ...
        timeout: 15e3,
        success: function (d) {
            _vidok_parseM3U(d, cb);
        },
        error: function () {
            $.ajax({
                url: host + "/m3u/cp.php",
                data: { url: "@" + _vidok_cfg.m3u },
                method: "post",
                dataType: "text",
                timeout: 15e3,
                success: function (d) {
...
function _vidok_xtream(cb) {
    ...
    var api =
        ...
        encodeURIComponent(_vidok_cfg.user) +
        "&password=" +
        encodeURIComponent(_vidok_cfg.pass);
...
function duneAddSettings(e) {
    ...
    popupDetail.splice(e, 1, _("Vidok.TV settings"));
    popupActions.splice(e, 1, _vidok_edit);
    var idx = popupActions.indexOf(_vidok_edit);
    if (idx > -1) {
        var lbl = _("Vidok.TV settings");
        if (_vidok_cfg.server && _vidok_cfg.user)
            lbl +=
                ": " +
                _vidok_cfg.server.replace(/^https?:\/\//, "").split("/")[0] +
                " (" +
                _vidok_cfg.user +
                ")";
        else if (_vidok_cfg.m3u)
            lbl += ": " + _vidok_cfg.m3u.substr(0, 40) + "...";
        popupArray[idx] = lbl;
    }
}
function _vidok_edit() {
    ...
    listCaption.innerHTML = _("Vidok.TV");
