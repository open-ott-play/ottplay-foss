...
var _vidok_cfg = { server: '', user: '', pass: '', m3u: '' };
function _vidok_load() {
    try { var d = providerGetItem('cfg'); if (d) _vidok_cfg = JSON.parse(d); } catch(e) {}
    if (!_vidok_cfg.server && !_vidok_cfg.m3u) _vidok_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _vidok_save() { providerSetItem('cfg', JSON.stringify(_vidok_cfg)); }
...
function getChanelsArray(cb) {
    _vidok_load();
    if (_vidok_cfg.server && _vidok_cfg.user && _vidok_cfg.pass) _vidok_xtream(cb);
    else if (_vidok_cfg.m3u) _vidok_m3u(cb);
    else { alert(_('Configure Vidok.TV in Settings -> Provider Settings')); cb(); }
}
function _vidok_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _vidok_cfg.m3u, timeout: 15e3,
        success: function(d) { _vidok_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _vidok_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _vidok_parseM3U(d, cb); },
...
function _vidok_parseM3U(data, cb) {
...
function _vidok_xtream(cb) {
    $(launch_id).append(_('Loading from API...'));
    var api = _vidok_cfg.server + '/player_api.php?username=' + encodeURIComponent(_vidok_cfg.user) + '&password=' + encodeURIComponent(_vidok_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _vidok_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _vidok_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _vidok_cfg.server + '/live/' + encodeURIComponent(_vidok_cfg.user) + '/' + encodeURIComponent(_vidok_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _vidok_cfg.m3u = _vidok_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_vidok_cfg.user) + '&password=' + encodeURIComponent(_vidok_cfg.pass) + '&type=m3u_plus&output=ts';
        _vidok_m3u(cb);
    });
}
function duneAddSettings(e) {
    _vidok_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('Vidok.TV settings'));
    popupActions.splice(e, 1, _vidok_edit);
    var idx = popupActions.indexOf(_vidok_edit);
    if (idx > -1) {
        var lbl = _('Vidok.TV settings');
        if (_vidok_cfg.server && _vidok_cfg.user) lbl += ': ' + _vidok_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _vidok_cfg.user + ')';
        else if (_vidok_cfg.m3u) lbl += ': ' + _vidok_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _vidok_edit() {
    selIndex = 0; _vidok_load();
    var srv = _vidok_cfg.server, usr = _vidok_cfg.user, pwd = _vidok_cfg.pass, m3u = _vidok_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _vidok_cfg.server = srv; _vidok_cfg.user = usr; _vidok_cfg.pass = pwd; _vidok_cfg.m3u = m3u; _vidok_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('Vidok.TV');
