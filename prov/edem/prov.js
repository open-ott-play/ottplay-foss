...
function loadEdemParams() {
    try { var d = providerGetItem('edem_data'); if (d) edem = JSON.parse(d); } catch(e) {}
    if (!edem.server && !edem.m3u_url) edem = { server: '', username: '', password: '', m3u_url: '' };
}

function saveEdemParams() {
    providerSetItem('edem_data', JSON.stringify({ server: edem.server, username: edem.username, password: edem.password, m3u_url: edem.m3u_url }));
}
    ...
    $.ajax({
        ...
        error: function(e, r, t) {
            ...
            $.ajax({
                ...
                dataType: 'text',
                timeout: 15e3,
                success: function(data) { parseM3U(data, callback); },
                error: function() { alert(_('Failed to load playlist!')); callback(); }
            });
        }
    });
}
    ...
    try {
        ...
        lines.forEach(function(block) {
            ...
            for (var i = 1; i < parts.length; i++) {
                if (parts[i].trim() && parts[i].trim()[0] !== '#') { url = parts[i].trim(); break; }
            }
            if (!url) return;
            var name = '???';
            ...
            if (cList.indexOf(h) === -1) {
                ...
                chanels[h] = {
                    ...
                    rec: 0, time: 0, time_to: 0,
                    url: url, logo: logo, epg: '', tn: name, ca: '', caso: ''
                };
            }
        });
    } catch(e) { console.error(e); alert(_('Failed to parse playlist!')); }
    callback();
}

function loadFromXtreamAPI(callback) {
    $(launch_id).append(_('Loading from Edem API...'));
    var apiUrl = edem.server + '/player_api.php?username=' + encodeURIComponent(edem.username) +
                 '&password=' + encodeURIComponent(edem.password);
    $.ajax({
        type: 'GET', url: apiUrl, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) {
            edem.m3u_url = apiUrl.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts';
            loadFromM3U(callback);
            return;
        }
        var catMap = {};
        if (r.categories) r.categories.forEach(function(c) { catMap[c.category_id] = c.category_name || 'Unknown'; });
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                ...
                chanels[h] = {
                    ...
                    rec: 0, time: 0, time_to: 0,
                    url: edem.server + '/live/' + encodeURIComponent(edem.username) + '/' +
                         encodeURIComponent(edem.password) + '/' + s.stream_id + '.m3u8',
                    logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: ''
                };
            }
        });
        callback();
        ...
        edem.m3u_url = edem.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(edem.username) +
                       '&password=' + encodeURIComponent(edem.password) + '&type=m3u_plus&output=ts';
        loadFromM3U(callback);
    });
}
...
function editEdemSettings() {
    selIndex = 0;
    loadEdemParams();
    var srv = edem.server, usr = edem.username, pwd = edem.password, m3u = edem.m3u_url;
    function buildList() {
        listArray = [
            _('API Server') + ': ' + (srv || ''),
            _('Username') + ': ' + (usr || ''),
            ...
            case keys.ENTER:
                switch (selIndex) {
                    case 0:
                        ...
                        editvar = srv;
                        setEdit = function() { srv = editvar.trim(); buildList(); showPage(); };
                        showEditKey(keys.ENTER); return true;
                    case 1:
                        editCaption = _('Enter username');
                        editvar = usr;
                        setEdit = function() { usr = editvar.trim(); buildList(); showPage(); };
                        showEditKey(keys.ENTER); return true;
                    case 2:
                        editCaption = _('Enter password');
                        editvar = pwd;
                        setEdit = function() { pwd = editvar.trim(); buildList(); showPage(); };
                        showEditKey(keys.ENTER); return true;
                    case 3:
                        editCaption = _('Enter M3U playlist URL');
                        editvar = m3u;
                        setEdit = function() { m3u = editvar.trim(); buildList(); showPage(); };
                        showEditKey(keys.ENTER); return true;
                    case 5:
                        edem.server = srv; edem.username = usr; edem.password = pwd; edem.m3u_url = m3u;
                        saveEdemParams();
                        updateEdemPopup();
                        loadChannels();
                        return true;
