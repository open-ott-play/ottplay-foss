...
var _itv_cfg = { server: '', user: '', pass: '', m3u: '' };
function _itv_load() {
    try { var d = providerGetItem('cfg'); if (d) _itv_cfg = JSON.parse(d); } catch(e) {}
    if (!_itv_cfg.server && !_itv_cfg.m3u) _itv_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _itv_save() { providerSetItem('cfg', JSON.stringify(_itv_cfg)); }
...
function getChanelsArray(cb) {
    ...
    else { alert(_('Configure ITV.LIVE in Settings -> Provider Settings')); cb(); }
}
function _itv_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _itv_cfg.m3u, timeout: 15e3,
        success: function(d) { _itv_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _itv_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _itv_parseM3U(d, cb); },
...
function _itv_xtream(cb) {
    ...
    var api = _itv_cfg.server + '/player_api.php?username=' + encodeURIComponent(_itv_cfg.user) + '&password=' + encodeURIComponent(_itv_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _itv_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _itv_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _itv_cfg.server + '/live/' + encodeURIComponent(_itv_cfg.user) + '/' + encodeURIComponent(_itv_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _itv_cfg.m3u = _itv_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_itv_cfg.user) + '&password=' + encodeURIComponent(_itv_cfg.pass) + '&type=m3u_plus&output=ts';
        _itv_m3u(cb);
    });
}
function duneAddSettings(e) {
    _itv_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('ITV.LIVE settings'));
    popupActions.splice(e, 1, _itv_edit);
    var idx = popupActions.indexOf(_itv_edit);
    if (idx > -1) {
        var lbl = _('ITV.LIVE settings');
        if (_itv_cfg.server && _itv_cfg.user) lbl += ': ' + _itv_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _itv_cfg.user + ')';
        else if (_itv_cfg.m3u) lbl += ': ' + _itv_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _itv_edit() {
    selIndex = 0; _itv_load();
    var srv = _itv_cfg.server, usr = _itv_cfg.user, pwd = _itv_cfg.pass, m3u = _itv_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _itv_cfg.server = srv; _itv_cfg.user = usr; _itv_cfg.pass = pwd; _itv_cfg.m3u = m3u; _itv_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('ITV.LIVE');
