...
function _dosug_load() {
    ...
    if (!(_dosug_cfg.server || _dosug_cfg.m3u))
        _dosug_cfg = { server: "", user: "", pass: "", m3u: "" };
}
...
function getChanelsArray(cb) {
    _dosug_load();
    if (_dosug_cfg.server && _dosug_cfg.user && _dosug_cfg.pass)
        _dosug_xtream(cb);
...
function _dosug_m3u(cb) {
    ...
    $.ajax({
        ...
        timeout: 15e3,
        success: function (d) {
            _dosug_parseM3U(d, cb);
        },
        error: function () {
            $.ajax({
                url: host + "/m3u/cp.php",
                data: { url: "@" + _dosug_cfg.m3u },
                method: "post",
                dataType: "text",
                timeout: 15e3,
                success: function (d) {
...
function _dosug_xtream(cb) {
    ...
    var api =
        ...
        encodeURIComponent(_dosug_cfg.user) +
        "&password=" +
        encodeURIComponent(_dosug_cfg.pass);
