...
var _dosug_cfg = { server: '', user: '', pass: '', m3u: '' };
function _dosug_load() {
    try { var d = providerGetItem('cfg'); if (d) _dosug_cfg = JSON.parse(d); } catch(e) {}
    if (!_dosug_cfg.server && !_dosug_cfg.m3u) _dosug_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _dosug_save() { providerSetItem('cfg', JSON.stringify(_dosug_cfg)); }
...
function getChanelsArray(cb) {
    _dosug_load();
    if (_dosug_cfg.server && _dosug_cfg.user && _dosug_cfg.pass) _dosug_xtream(cb);
    else if (_dosug_cfg.m3u) _dosug_m3u(cb);
    else { alert(_('Configure TV DOSUG in Settings -> Provider Settings')); cb(); }
}
function _dosug_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _dosug_cfg.m3u, timeout: 15e3,
        success: function(d) { _dosug_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _dosug_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _dosug_parseM3U(d, cb); },
...
function _dosug_xtream(cb) {
    ...
    var api = _dosug_cfg.server + '/player_api.php?username=' + encodeURIComponent(_dosug_cfg.user) + '&password=' + encodeURIComponent(_dosug_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _dosug_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _dosug_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _dosug_cfg.server + '/live/' + encodeURIComponent(_dosug_cfg.user) + '/' + encodeURIComponent(_dosug_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _dosug_cfg.m3u = _dosug_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_dosug_cfg.user) + '&password=' + encodeURIComponent(_dosug_cfg.pass) + '&type=m3u_plus&output=ts';
        _dosug_m3u(cb);
    });
}
function duneAddSettings(e) {
    _dosug_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('TV DOSUG settings'));
    popupActions.splice(e, 1, _dosug_edit);
    var idx = popupActions.indexOf(_dosug_edit);
    if (idx > -1) {
        var lbl = _('TV DOSUG settings');
        if (_dosug_cfg.server && _dosug_cfg.user) lbl += ': ' + _dosug_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _dosug_cfg.user + ')';
        else if (_dosug_cfg.m3u) lbl += ': ' + _dosug_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _dosug_edit() {
    selIndex = 0; _dosug_load();
    var srv = _dosug_cfg.server, usr = _dosug_cfg.user, pwd = _dosug_cfg.pass, m3u = _dosug_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _dosug_cfg.server = srv; _dosug_cfg.user = usr; _dosug_cfg.pass = pwd; _dosug_cfg.m3u = m3u; _dosug_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('TV DOSUG');
