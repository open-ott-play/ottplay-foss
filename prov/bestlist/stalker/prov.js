...
var _bestlist_stalker_cfg = { server: '', user: '', pass: '', m3u: '' };
function _bestlist_stalker_load() {
    try { var d = providerGetItem('cfg'); if (d) _bestlist_stalker_cfg = JSON.parse(d); } catch(e) {}
    if (!_bestlist_stalker_cfg.server && !_bestlist_stalker_cfg.m3u) _bestlist_stalker_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _bestlist_stalker_save() { providerSetItem('cfg', JSON.stringify(_bestlist_stalker_cfg)); }
...
function getChanelsArray(cb) {
    ...
    if (_bestlist_stalker_cfg.server && _bestlist_stalker_cfg.user && _bestlist_stalker_cfg.pass) _bestlist_stalker_xtream(cb);
    else if (_bestlist_stalker_cfg.m3u) _bestlist_stalker_m3u(cb);
    else { alert(_('Configure BEST LiST IPTV [Stalker] in Settings -> Provider Settings')); cb(); }
}
function _bestlist_stalker_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _bestlist_stalker_cfg.m3u, timeout: 15e3,
        success: function(d) { _bestlist_stalker_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _bestlist_stalker_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _bestlist_stalker_parseM3U(d, cb); },
...
function _bestlist_stalker_xtream(cb) {
    ...
    var api = _bestlist_stalker_cfg.server + '/player_api.php?username=' + encodeURIComponent(_bestlist_stalker_cfg.user) + '&password=' + encodeURIComponent(_bestlist_stalker_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _bestlist_stalker_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _bestlist_stalker_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _bestlist_stalker_cfg.server + '/live/' + encodeURIComponent(_bestlist_stalker_cfg.user) + '/' + encodeURIComponent(_bestlist_stalker_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _bestlist_stalker_cfg.m3u = _bestlist_stalker_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_bestlist_stalker_cfg.user) + '&password=' + encodeURIComponent(_bestlist_stalker_cfg.pass) + '&type=m3u_plus&output=ts';
        _bestlist_stalker_m3u(cb);
    });
}
function duneAddSettings(e) {
    _bestlist_stalker_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('BEST LiST IPTV [Stalker] settings'));
    ...
    if (idx > -1) {
        ...
        if (_bestlist_stalker_cfg.server && _bestlist_stalker_cfg.user) lbl += ': ' + _bestlist_stalker_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _bestlist_stalker_cfg.user + ')';
        else if (_bestlist_stalker_cfg.m3u) lbl += ': ' + _bestlist_stalker_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _bestlist_stalker_edit() {
    selIndex = 0; _bestlist_stalker_load();
    var srv = _bestlist_stalker_cfg.server, usr = _bestlist_stalker_cfg.user, pwd = _bestlist_stalker_cfg.pass, m3u = _bestlist_stalker_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _bestlist_stalker_cfg.server = srv; _bestlist_stalker_cfg.user = usr; _bestlist_stalker_cfg.pass = pwd; _bestlist_stalker_cfg.m3u = m3u; _bestlist_stalker_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('BEST LiST IPTV [Stalker]');
