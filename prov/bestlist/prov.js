...
function _bestlist_load() {
    try {
        ...
        if (d) _bestlist_cfg = JSON.parse(d);
    } catch (e) {}
    if (!(_bestlist_cfg.server || _bestlist_cfg.m3u))
        _bestlist_cfg = { server: "", user: "", pass: "", m3u: "" };
}
function _bestlist_save() {
    providerSetItem("cfg", JSON.stringify(_bestlist_cfg));
}
...
function getChanelsArray(cb) {
    _bestlist_load();
    if (_bestlist_cfg.server && _bestlist_cfg.user && _bestlist_cfg.pass)
        _bestlist_xtream(cb);
...
function _bestlist_m3u(cb) {
    ...
    $.ajax({
        url: _bestlist_cfg.m3u,
        timeout: 15e3,
        success: function (d) {
            _bestlist_parseM3U(d, cb);
        },
        error: function () {
            $.ajax({
                url: host + "/m3u/cp.php",
                data: { url: "@" + _bestlist_cfg.m3u },
                method: "post",
                dataType: "text",
                timeout: 15e3,
                success: function (d) {
...
function _bestlist_xtream(cb) {
    ...
    var api =
        _bestlist_cfg.server +
        "/player_api.php?username=" +
        encodeURIComponent(_bestlist_cfg.user) +
        "&password=" +
        encodeURIComponent(_bestlist_cfg.pass);
...
function duneAddSettings(e) {
    ...
    popupDetail.splice(e, 1, _("BEST LiST IPTV [HLS Playlist] settings"));
    ...
    if (idx > -1) {
        ...
        if (_bestlist_cfg.server && _bestlist_cfg.user)
            lbl +=
                ": " +
                _bestlist_cfg.server.replace(/^https?:\/\//, "").split("/")[0] +
                " (" +
                _bestlist_cfg.user +
                ")";
        else if (_bestlist_cfg.m3u)
            lbl += ": " + _bestlist_cfg.m3u.substr(0, 40) + "...";
        popupArray[idx] = lbl;
    }
}
function _bestlist_edit() {
            ...
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 3:
                        ...
                        return true;
                    case 5:
                        _bestlist_cfg.server = srv;
                        _bestlist_cfg.user = usr;
                        _bestlist_cfg.pass = pwd;
                        _bestlist_cfg.m3u = m3u;
                        _bestlist_save();
                        duneAddSettings(0);
                        loadChannels();
                        return true;
    ...
    listCaption.innerHTML = _("BEST LiST IPTV [HLS Playlist]");
