...
var _bestlist_cfg = { server: '', user: '', pass: '', m3u: '' };
function _bestlist_load() {
    try { var d = providerGetItem('cfg'); if (d) _bestlist_cfg = JSON.parse(d); } catch(e) {}
    if (!_bestlist_cfg.server && !_bestlist_cfg.m3u) _bestlist_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _bestlist_save() { providerSetItem('cfg', JSON.stringify(_bestlist_cfg)); }
...
function getChanelsArray(cb) {
    _bestlist_load();
    if (_bestlist_cfg.server && _bestlist_cfg.user && _bestlist_cfg.pass) _bestlist_xtream(cb);
    else if (_bestlist_cfg.m3u) _bestlist_m3u(cb);
    else { alert(_('Configure BEST LiST IPTV [HLS Playlist] in Settings -> Provider Settings')); cb(); }
}
function _bestlist_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _bestlist_cfg.m3u, timeout: 15e3,
        success: function(d) { _bestlist_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _bestlist_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _bestlist_parseM3U(d, cb); },
...
function _bestlist_parseM3U(data, cb) {
...
function _bestlist_xtream(cb) {
    $(launch_id).append(_('Loading from API...'));
    var api = _bestlist_cfg.server + '/player_api.php?username=' + encodeURIComponent(_bestlist_cfg.user) + '&password=' + encodeURIComponent(_bestlist_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _bestlist_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _bestlist_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _bestlist_cfg.server + '/live/' + encodeURIComponent(_bestlist_cfg.user) + '/' + encodeURIComponent(_bestlist_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _bestlist_cfg.m3u = _bestlist_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_bestlist_cfg.user) + '&password=' + encodeURIComponent(_bestlist_cfg.pass) + '&type=m3u_plus&output=ts';
        _bestlist_m3u(cb);
    });
}
function duneAddSettings(e) {
    _bestlist_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('BEST LiST IPTV [HLS Playlist] settings'));
    popupActions.splice(e, 1, _bestlist_edit);
    var idx = popupActions.indexOf(_bestlist_edit);
    if (idx > -1) {
        var lbl = _('BEST LiST IPTV [HLS Playlist] settings');
        if (_bestlist_cfg.server && _bestlist_cfg.user) lbl += ': ' + _bestlist_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _bestlist_cfg.user + ')';
        else if (_bestlist_cfg.m3u) lbl += ': ' + _bestlist_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _bestlist_edit() {
    selIndex = 0; _bestlist_load();
    var srv = _bestlist_cfg.server, usr = _bestlist_cfg.user, pwd = _bestlist_cfg.pass, m3u = _bestlist_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _bestlist_cfg.server = srv; _bestlist_cfg.user = usr; _bestlist_cfg.pass = pwd; _bestlist_cfg.m3u = m3u; _bestlist_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('BEST LiST IPTV [HLS Playlist]');
