...
var _ottclub_cfg = { server: '', user: '', pass: '', m3u: '' };
function _ottclub_load() {
    try { var d = providerGetItem('cfg'); if (d) _ottclub_cfg = JSON.parse(d); } catch(e) {}
    if (!_ottclub_cfg.server && !_ottclub_cfg.m3u) _ottclub_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _ottclub_save() { providerSetItem('cfg', JSON.stringify(_ottclub_cfg)); }
...
function getChanelsArray(cb) {
    _ottclub_load();
    if (_ottclub_cfg.server && _ottclub_cfg.user && _ottclub_cfg.pass) _ottclub_xtream(cb);
    else if (_ottclub_cfg.m3u) _ottclub_m3u(cb);
    else { alert(_('Configure OTTCLUB in Settings -> Provider Settings')); cb(); }
}
function _ottclub_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _ottclub_cfg.m3u, timeout: 15e3,
        success: function(d) { _ottclub_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _ottclub_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _ottclub_parseM3U(d, cb); },
...
function _ottclub_xtream(cb) {
    ...
    var api = _ottclub_cfg.server + '/player_api.php?username=' + encodeURIComponent(_ottclub_cfg.user) + '&password=' + encodeURIComponent(_ottclub_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _ottclub_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _ottclub_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _ottclub_cfg.server + '/live/' + encodeURIComponent(_ottclub_cfg.user) + '/' + encodeURIComponent(_ottclub_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _ottclub_cfg.m3u = _ottclub_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_ottclub_cfg.user) + '&password=' + encodeURIComponent(_ottclub_cfg.pass) + '&type=m3u_plus&output=ts';
        _ottclub_m3u(cb);
    });
}
function duneAddSettings(e) {
    _ottclub_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('OTTCLUB settings'));
    popupActions.splice(e, 1, _ottclub_edit);
    var idx = popupActions.indexOf(_ottclub_edit);
    if (idx > -1) {
        var lbl = _('OTTCLUB settings');
        if (_ottclub_cfg.server && _ottclub_cfg.user) lbl += ': ' + _ottclub_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _ottclub_cfg.user + ')';
        else if (_ottclub_cfg.m3u) lbl += ': ' + _ottclub_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _ottclub_edit() {
    selIndex = 0; _ottclub_load();
    var srv = _ottclub_cfg.server, usr = _ottclub_cfg.user, pwd = _ottclub_cfg.pass, m3u = _ottclub_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _ottclub_cfg.server = srv; _ottclub_cfg.user = usr; _ottclub_cfg.pass = pwd; _ottclub_cfg.m3u = m3u; _ottclub_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('OTTCLUB');
