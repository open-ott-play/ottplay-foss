...
function _1ott_load() {
    try { var d = providerGetItem('cfg'); if (d) _1ott_cfg = JSON.parse(d); } catch(e) {}
    if (!_1ott_cfg.server && !_1ott_cfg.m3u) _1ott_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _1ott_save() { providerSetItem('cfg', JSON.stringify(_1ott_cfg)); }
...
function getChanelsArray(cb) {
    ...
    else { alert(_('Configure 1OTT.NET in Settings -> Provider Settings')); cb(); }
}
function _1ott_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _1ott_cfg.m3u, timeout: 15e3,
        success: function(d) { _1ott_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _1ott_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _1ott_parseM3U(d, cb); },
...
function _1ott_xtream(cb) {
    ...
    var api = _1ott_cfg.server + '/player_api.php?username=' + encodeURIComponent(_1ott_cfg.user) + '&password=' + encodeURIComponent(_1ott_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _1ott_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _1ott_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _1ott_cfg.server + '/live/' + encodeURIComponent(_1ott_cfg.user) + '/' + encodeURIComponent(_1ott_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _1ott_cfg.m3u = _1ott_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_1ott_cfg.user) + '&password=' + encodeURIComponent(_1ott_cfg.pass) + '&type=m3u_plus&output=ts';
        _1ott_m3u(cb);
    });
}
function duneAddSettings(e) {
    _1ott_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('1OTT.NET settings'));
    popupActions.splice(e, 1, _1ott_edit);
    var idx = popupActions.indexOf(_1ott_edit);
    if (idx > -1) {
        var lbl = _('1OTT.NET settings');
        if (_1ott_cfg.server && _1ott_cfg.user) lbl += ': ' + _1ott_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _1ott_cfg.user + ')';
        else if (_1ott_cfg.m3u) lbl += ': ' + _1ott_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _1ott_edit() {
    selIndex = 0; _1ott_load();
    var srv = _1ott_cfg.server, usr = _1ott_cfg.user, pwd = _1ott_cfg.pass, m3u = _1ott_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _1ott_cfg.server = srv; _1ott_cfg.user = usr; _1ott_cfg.pass = pwd; _1ott_cfg.m3u = m3u; _1ott_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('1OTT.NET');
