...
var _shara_club_cfg = { m3u: "", pass: "", server: "", user: "" };
function _shara_club_load() {
    try {
        var d = providerGetItem("cfg");
        if (d) _shara_club_cfg = JSON.parse(d);
    } catch (e) {}
    if (!(_shara_club_cfg.server || _shara_club_cfg.m3u))
        _shara_club_cfg = { m3u: "", pass: "", server: "", user: "" };
}
function _shara_club_save() {
    providerSetItem("cfg", JSON.stringify(_shara_club_cfg));
}
...
function getChanelsArray(cb) {
    _shara_club_load();
    if (_shara_club_cfg.server && _shara_club_cfg.user && _shara_club_cfg.pass)
        _shara_club_xtream(cb);
...
function _shara_club_xtream(cb) {
    ...
    var api =
        _shara_club_cfg.server +
        "/player_api.php?username=" +
        encodeURIComponent(_shara_club_cfg.user) +
        "&password=" +
        encodeURIComponent(_shara_club_cfg.pass);
    $.ajax({ dataType: "json", timeout: 15e3, type: "GET", url: api })
        ...
        .fail(function () {
            _shara_club_cfg.m3u =
                _shara_club_cfg.server.replace(/\/+$/, "") +
                "/get.php?username=" +
                encodeURIComponent(_shara_club_cfg.user) +
                "&password=" +
                encodeURIComponent(_shara_club_cfg.pass) +
...
function duneAddSettings(e) {
    ...
    popupDetail.splice(e, 1, _("Shara.club (ClubTV.pro) settings"));
    ...
    if (idx > -1) {
        ...
        if (_shara_club_cfg.server && _shara_club_cfg.user)
            lbl +=
                ": " +
                _shara_club_cfg.server
                    .replace(/^https?:\/\//, "")
                    .split("/")[0] +
                " (" +
                _shara_club_cfg.user +
                ")";
        else if (_shara_club_cfg.m3u)
            lbl += ": " + _shara_club_cfg.m3u.substr(0, 40) + "...";
        popupArray[idx] = lbl;
    }
}
function _shara_club_edit() {
    selIndex = 0;
    _shara_club_load();
    var srv = _shara_club_cfg.server,
        usr = _shara_club_cfg.user,
        pwd = _shara_club_cfg.pass,
        m3u = _shara_club_cfg.m3u;
    function bl() {
        listArray = [
            _("Server") + ": " + (srv || ""),
            _("Login") + ": " + (usr || ""),
            ...
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 3:
                        ...
                        return true;
                    case 5:
                        _shara_club_cfg.server = srv;
                        _shara_club_cfg.user = usr;
                        _shara_club_cfg.pass = pwd;
                        _shara_club_cfg.m3u = m3u;
                        _shara_club_save();
                        duneAddSettings(0);
                        loadChannels();
                        return true;
    ...
    listCaption.innerHTML = _("Shara.club (ClubTV.pro)");
