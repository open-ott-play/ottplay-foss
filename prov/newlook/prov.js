...
var _newlook_cfg = { server: '', user: '', pass: '', m3u: '' };
function _newlook_load() {
    try { var d = providerGetItem('cfg'); if (d) _newlook_cfg = JSON.parse(d); } catch(e) {}
    if (!_newlook_cfg.server && !_newlook_cfg.m3u) _newlook_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _newlook_save() { providerSetItem('cfg', JSON.stringify(_newlook_cfg)); }
...
function getChanelsArray(cb) {
    _newlook_load();
    if (_newlook_cfg.server && _newlook_cfg.user && _newlook_cfg.pass) _newlook_xtream(cb);
    else if (_newlook_cfg.m3u) _newlook_m3u(cb);
    else { alert(_('Configure New Look in Settings -> Provider Settings')); cb(); }
}
function _newlook_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _newlook_cfg.m3u, timeout: 15e3,
        success: function(d) { _newlook_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _newlook_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _newlook_parseM3U(d, cb); },
...
function _newlook_parseM3U(data, cb) {
...
function _newlook_xtream(cb) {
    $(launch_id).append(_('Loading from API...'));
    var api = _newlook_cfg.server + '/player_api.php?username=' + encodeURIComponent(_newlook_cfg.user) + '&password=' + encodeURIComponent(_newlook_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _newlook_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _newlook_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _newlook_cfg.server + '/live/' + encodeURIComponent(_newlook_cfg.user) + '/' + encodeURIComponent(_newlook_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _newlook_cfg.m3u = _newlook_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_newlook_cfg.user) + '&password=' + encodeURIComponent(_newlook_cfg.pass) + '&type=m3u_plus&output=ts';
        _newlook_m3u(cb);
    });
}
function duneAddSettings(e) {
    _newlook_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('New Look settings'));
    popupActions.splice(e, 1, _newlook_edit);
    var idx = popupActions.indexOf(_newlook_edit);
    if (idx > -1) {
        var lbl = _('New Look settings');
        if (_newlook_cfg.server && _newlook_cfg.user) lbl += ': ' + _newlook_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _newlook_cfg.user + ')';
        else if (_newlook_cfg.m3u) lbl += ': ' + _newlook_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _newlook_edit() {
    selIndex = 0; _newlook_load();
    var srv = _newlook_cfg.server, usr = _newlook_cfg.user, pwd = _newlook_cfg.pass, m3u = _newlook_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _newlook_cfg.server = srv; _newlook_cfg.user = usr; _newlook_cfg.pass = pwd; _newlook_cfg.m3u = m3u; _newlook_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('New Look');
