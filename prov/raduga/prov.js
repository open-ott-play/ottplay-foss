...
function _raduga_load() {
    try { var d = providerGetItem('cfg'); if (d) _raduga_cfg = JSON.parse(d); } catch(e) {}
    if (!_raduga_cfg.server && !_raduga_cfg.m3u) _raduga_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _raduga_save() { providerSetItem('cfg', JSON.stringify(_raduga_cfg)); }
...
function getChanelsArray(cb) {
    _raduga_load();
    if (_raduga_cfg.server && _raduga_cfg.user && _raduga_cfg.pass) _raduga_xtream(cb);
    else if (_raduga_cfg.m3u) _raduga_m3u(cb);
    else { alert(_('Configure Радуга in Settings -> Provider Settings')); cb(); }
}
function _raduga_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _raduga_cfg.m3u, timeout: 15e3,
        success: function(d) { _raduga_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _raduga_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _raduga_parseM3U(d, cb); },
...
function _raduga_xtream(cb) {
    ...
    var api = _raduga_cfg.server + '/player_api.php?username=' + encodeURIComponent(_raduga_cfg.user) + '&password=' + encodeURIComponent(_raduga_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _raduga_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _raduga_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _raduga_cfg.server + '/live/' + encodeURIComponent(_raduga_cfg.user) + '/' + encodeURIComponent(_raduga_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _raduga_cfg.m3u = _raduga_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_raduga_cfg.user) + '&password=' + encodeURIComponent(_raduga_cfg.pass) + '&type=m3u_plus&output=ts';
        _raduga_m3u(cb);
    });
}
function duneAddSettings(e) {
    _raduga_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('Радуга settings'));
    popupActions.splice(e, 1, _raduga_edit);
    var idx = popupActions.indexOf(_raduga_edit);
    if (idx > -1) {
        var lbl = _('Радуга settings');
        if (_raduga_cfg.server && _raduga_cfg.user) lbl += ': ' + _raduga_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _raduga_cfg.user + ')';
        else if (_raduga_cfg.m3u) lbl += ': ' + _raduga_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _raduga_edit() {
    selIndex = 0; _raduga_load();
    var srv = _raduga_cfg.server, usr = _raduga_cfg.user, pwd = _raduga_cfg.pass, m3u = _raduga_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _raduga_cfg.server = srv; _raduga_cfg.user = usr; _raduga_cfg.pass = pwd; _raduga_cfg.m3u = m3u; _raduga_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('Радуга');
