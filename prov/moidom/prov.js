...
function _moidom_load() {
    ...
    if (!(_moidom_cfg.server || _moidom_cfg.m3u))
        _moidom_cfg = { server: "", user: "", pass: "", m3u: "" };
}
...
function getChanelsArray(cb) {
    _moidom_load();
    if (_moidom_cfg.server && _moidom_cfg.user && _moidom_cfg.pass)
        _moidom_xtream(cb);
...
function _moidom_m3u(cb) {
    ...
    $.ajax({
        ...
        timeout: 15e3,
        success: function (d) {
            _moidom_parseM3U(d, cb);
        },
        error: function () {
            $.ajax({
                url: host + "/m3u/cp.php",
                data: { url: "@" + _moidom_cfg.m3u },
                method: "post",
                dataType: "text",
                timeout: 15e3,
                success: function (d) {
...
function _moidom_xtream(cb) {
    ...
    var api =
        ...
        encodeURIComponent(_moidom_cfg.user) +
        "&password=" +
        encodeURIComponent(_moidom_cfg.pass);
