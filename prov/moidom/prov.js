...
var _moidom_cfg = { server: '', user: '', pass: '', m3u: '' };
function _moidom_load() {
    try { var d = providerGetItem('cfg'); if (d) _moidom_cfg = JSON.parse(d); } catch(e) {}
    if (!_moidom_cfg.server && !_moidom_cfg.m3u) _moidom_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _moidom_save() { providerSetItem('cfg', JSON.stringify(_moidom_cfg)); }
...
function getChanelsArray(cb) {
    _moidom_load();
    if (_moidom_cfg.server && _moidom_cfg.user && _moidom_cfg.pass) _moidom_xtream(cb);
    else if (_moidom_cfg.m3u) _moidom_m3u(cb);
    else { alert(_('Configure МойДом in Settings -> Provider Settings')); cb(); }
}
function _moidom_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _moidom_cfg.m3u, timeout: 15e3,
        success: function(d) { _moidom_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _moidom_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _moidom_parseM3U(d, cb); },
...
function _moidom_xtream(cb) {
    ...
    var api = _moidom_cfg.server + '/player_api.php?username=' + encodeURIComponent(_moidom_cfg.user) + '&password=' + encodeURIComponent(_moidom_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _moidom_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _moidom_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _moidom_cfg.server + '/live/' + encodeURIComponent(_moidom_cfg.user) + '/' + encodeURIComponent(_moidom_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _moidom_cfg.m3u = _moidom_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_moidom_cfg.user) + '&password=' + encodeURIComponent(_moidom_cfg.pass) + '&type=m3u_plus&output=ts';
        _moidom_m3u(cb);
    });
}
function duneAddSettings(e) {
    _moidom_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('МойДом settings'));
    popupActions.splice(e, 1, _moidom_edit);
    var idx = popupActions.indexOf(_moidom_edit);
    if (idx > -1) {
        var lbl = _('МойДом settings');
        if (_moidom_cfg.server && _moidom_cfg.user) lbl += ': ' + _moidom_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _moidom_cfg.user + ')';
        else if (_moidom_cfg.m3u) lbl += ': ' + _moidom_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _moidom_edit() {
    selIndex = 0; _moidom_load();
    var srv = _moidom_cfg.server, usr = _moidom_cfg.user, pwd = _moidom_cfg.pass, m3u = _moidom_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _moidom_cfg.server = srv; _moidom_cfg.user = usr; _moidom_cfg.pass = pwd; _moidom_cfg.m3u = m3u; _moidom_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('МойДом');
