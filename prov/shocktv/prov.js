...
function _shocktv_load() {
    ...
    if (!(_shocktv_cfg.server || _shocktv_cfg.m3u))
        _shocktv_cfg = { server: "", user: "", pass: "", m3u: "" };
}
...
function getChanelsArray(cb) {
    _shocktv_load();
    if (_shocktv_cfg.server && _shocktv_cfg.user && _shocktv_cfg.pass)
        _shocktv_xtream(cb);
...
function _shocktv_m3u(cb) {
    ...
    $.ajax({
        url: _shocktv_cfg.m3u,
        timeout: 15e3,
        success: function (d) {
            _shocktv_parseM3U(d, cb);
        },
        error: function () {
            $.ajax({
                url: host + "/m3u/cp.php",
                data: { url: "@" + _shocktv_cfg.m3u },
                method: "post",
                dataType: "text",
                timeout: 15e3,
                success: function (d) {
...
function _shocktv_xtream(cb) {
    ...
    var api =
        ...
        encodeURIComponent(_shocktv_cfg.user) +
        "&password=" +
        encodeURIComponent(_shocktv_cfg.pass);
