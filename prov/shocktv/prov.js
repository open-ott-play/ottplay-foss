...
var _shocktv_cfg = { server: '', user: '', pass: '', m3u: '' };
function _shocktv_load() {
    try { var d = providerGetItem('cfg'); if (d) _shocktv_cfg = JSON.parse(d); } catch(e) {}
    if (!_shocktv_cfg.server && !_shocktv_cfg.m3u) _shocktv_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _shocktv_save() { providerSetItem('cfg', JSON.stringify(_shocktv_cfg)); }
...
function getChanelsArray(cb) {
    _shocktv_load();
    if (_shocktv_cfg.server && _shocktv_cfg.user && _shocktv_cfg.pass) _shocktv_xtream(cb);
    else if (_shocktv_cfg.m3u) _shocktv_m3u(cb);
    else { alert(_('Configure ShockTv in Settings -> Provider Settings')); cb(); }
}
function _shocktv_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _shocktv_cfg.m3u, timeout: 15e3,
        success: function(d) { _shocktv_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _shocktv_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _shocktv_parseM3U(d, cb); },
...
function _shocktv_xtream(cb) {
    ...
    var api = _shocktv_cfg.server + '/player_api.php?username=' + encodeURIComponent(_shocktv_cfg.user) + '&password=' + encodeURIComponent(_shocktv_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _shocktv_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _shocktv_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _shocktv_cfg.server + '/live/' + encodeURIComponent(_shocktv_cfg.user) + '/' + encodeURIComponent(_shocktv_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _shocktv_cfg.m3u = _shocktv_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_shocktv_cfg.user) + '&password=' + encodeURIComponent(_shocktv_cfg.pass) + '&type=m3u_plus&output=ts';
        _shocktv_m3u(cb);
    });
}
function duneAddSettings(e) {
    _shocktv_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('ShockTv settings'));
    popupActions.splice(e, 1, _shocktv_edit);
    var idx = popupActions.indexOf(_shocktv_edit);
    if (idx > -1) {
        var lbl = _('ShockTv settings');
        if (_shocktv_cfg.server && _shocktv_cfg.user) lbl += ': ' + _shocktv_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _shocktv_cfg.user + ')';
        else if (_shocktv_cfg.m3u) lbl += ': ' + _shocktv_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _shocktv_edit() {
    selIndex = 0; _shocktv_load();
    var srv = _shocktv_cfg.server, usr = _shocktv_cfg.user, pwd = _shocktv_cfg.pass, m3u = _shocktv_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _shocktv_cfg.server = srv; _shocktv_cfg.user = usr; _shocktv_cfg.pass = pwd; _shocktv_cfg.m3u = m3u; _shocktv_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('ShockTv');
