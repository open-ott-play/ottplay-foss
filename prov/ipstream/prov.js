...
var _ipstream_cfg = { server: '', user: '', pass: '', m3u: '' };
function _ipstream_load() {
    try { var d = providerGetItem('cfg'); if (d) _ipstream_cfg = JSON.parse(d); } catch(e) {}
    if (!_ipstream_cfg.server && !_ipstream_cfg.m3u) _ipstream_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _ipstream_save() { providerSetItem('cfg', JSON.stringify(_ipstream_cfg)); }
...
function getChanelsArray(cb) {
    _ipstream_load();
    if (_ipstream_cfg.server && _ipstream_cfg.user && _ipstream_cfg.pass) _ipstream_xtream(cb);
    else if (_ipstream_cfg.m3u) _ipstream_m3u(cb);
    else { alert(_('Configure IpStream.one in Settings -> Provider Settings')); cb(); }
}
function _ipstream_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _ipstream_cfg.m3u, timeout: 15e3,
        success: function(d) { _ipstream_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _ipstream_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _ipstream_parseM3U(d, cb); },
...
function _ipstream_xtream(cb) {
    ...
    var api = _ipstream_cfg.server + '/player_api.php?username=' + encodeURIComponent(_ipstream_cfg.user) + '&password=' + encodeURIComponent(_ipstream_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _ipstream_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _ipstream_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _ipstream_cfg.server + '/live/' + encodeURIComponent(_ipstream_cfg.user) + '/' + encodeURIComponent(_ipstream_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _ipstream_cfg.m3u = _ipstream_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_ipstream_cfg.user) + '&password=' + encodeURIComponent(_ipstream_cfg.pass) + '&type=m3u_plus&output=ts';
        _ipstream_m3u(cb);
    });
}
function duneAddSettings(e) {
    _ipstream_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('IpStream.one settings'));
    popupActions.splice(e, 1, _ipstream_edit);
    var idx = popupActions.indexOf(_ipstream_edit);
    if (idx > -1) {
        var lbl = _('IpStream.one settings');
        if (_ipstream_cfg.server && _ipstream_cfg.user) lbl += ': ' + _ipstream_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _ipstream_cfg.user + ')';
        else if (_ipstream_cfg.m3u) lbl += ': ' + _ipstream_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _ipstream_edit() {
    selIndex = 0; _ipstream_load();
    var srv = _ipstream_cfg.server, usr = _ipstream_cfg.user, pwd = _ipstream_cfg.pass, m3u = _ipstream_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _ipstream_cfg.server = srv; _ipstream_cfg.user = usr; _ipstream_cfg.pass = pwd; _ipstream_cfg.m3u = m3u; _ipstream_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('IpStream.one');
