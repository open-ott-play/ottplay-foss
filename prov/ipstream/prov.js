...
function _ipstream_load() {
    ...
    if (!(_ipstream_cfg.server || _ipstream_cfg.m3u))
        _ipstream_cfg = { server: "", user: "", pass: "", m3u: "" };
}
...
function getChanelsArray(cb) {
    _ipstream_load();
    if (_ipstream_cfg.server && _ipstream_cfg.user && _ipstream_cfg.pass)
        _ipstream_xtream(cb);
...
function _ipstream_m3u(cb) {
    ...
    $.ajax({
        url: _ipstream_cfg.m3u,
        timeout: 15e3,
        success: function (d) {
            _ipstream_parseM3U(d, cb);
        },
        error: function () {
            $.ajax({
                url: host + "/m3u/cp.php",
                data: { url: "@" + _ipstream_cfg.m3u },
                method: "post",
                dataType: "text",
                timeout: 15e3,
                success: function (d) {
...
function _ipstream_xtream(cb) {
    ...
    var api =
        _ipstream_cfg.server +
        "/player_api.php?username=" +
        encodeURIComponent(_ipstream_cfg.user) +
        "&password=" +
        encodeURIComponent(_ipstream_cfg.pass);
