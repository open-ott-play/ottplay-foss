...
var _iptv_ott_ru_cfg = { server: '', user: '', pass: '', m3u: '' };
function _iptv_ott_ru_load() {
    try { var d = providerGetItem('cfg'); if (d) _iptv_ott_ru_cfg = JSON.parse(d); } catch(e) {}
    if (!_iptv_ott_ru_cfg.server && !_iptv_ott_ru_cfg.m3u) _iptv_ott_ru_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _iptv_ott_ru_save() { providerSetItem('cfg', JSON.stringify(_iptv_ott_ru_cfg)); }
...
function getChanelsArray(cb) {
    _iptv_ott_ru_load();
    if (_iptv_ott_ru_cfg.server && _iptv_ott_ru_cfg.user && _iptv_ott_ru_cfg.pass) _iptv_ott_ru_xtream(cb);
    else if (_iptv_ott_ru_cfg.m3u) _iptv_ott_ru_m3u(cb);
    else { alert(_('Configure VIP-IP.COM in Settings -> Provider Settings')); cb(); }
}
function _iptv_ott_ru_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _iptv_ott_ru_cfg.m3u, timeout: 15e3,
        success: function(d) { _iptv_ott_ru_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _iptv_ott_ru_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _iptv_ott_ru_parseM3U(d, cb); },
...
function _iptv_ott_ru_xtream(cb) {
    ...
    var api = _iptv_ott_ru_cfg.server + '/player_api.php?username=' + encodeURIComponent(_iptv_ott_ru_cfg.user) + '&password=' + encodeURIComponent(_iptv_ott_ru_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _iptv_ott_ru_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _iptv_ott_ru_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _iptv_ott_ru_cfg.server + '/live/' + encodeURIComponent(_iptv_ott_ru_cfg.user) + '/' + encodeURIComponent(_iptv_ott_ru_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _iptv_ott_ru_cfg.m3u = _iptv_ott_ru_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_iptv_ott_ru_cfg.user) + '&password=' + encodeURIComponent(_iptv_ott_ru_cfg.pass) + '&type=m3u_plus&output=ts';
        _iptv_ott_ru_m3u(cb);
    });
}
function duneAddSettings(e) {
    _iptv_ott_ru_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('VIP-IP.COM settings'));
    popupActions.splice(e, 1, _iptv_ott_ru_edit);
    var idx = popupActions.indexOf(_iptv_ott_ru_edit);
    if (idx > -1) {
        var lbl = _('VIP-IP.COM settings');
        if (_iptv_ott_ru_cfg.server && _iptv_ott_ru_cfg.user) lbl += ': ' + _iptv_ott_ru_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _iptv_ott_ru_cfg.user + ')';
        else if (_iptv_ott_ru_cfg.m3u) lbl += ': ' + _iptv_ott_ru_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _iptv_ott_ru_edit() {
    selIndex = 0; _iptv_ott_ru_load();
    var srv = _iptv_ott_ru_cfg.server, usr = _iptv_ott_ru_cfg.user, pwd = _iptv_ott_ru_cfg.pass, m3u = _iptv_ott_ru_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _iptv_ott_ru_cfg.server = srv; _iptv_ott_ru_cfg.user = usr; _iptv_ott_ru_cfg.pass = pwd; _iptv_ott_ru_cfg.m3u = m3u; _iptv_ott_ru_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('VIP-IP.COM');
