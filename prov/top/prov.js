...
var _top_cfg = { server: '', user: '', pass: '', m3u: '' };
function _top_load() {
    try { var d = providerGetItem('cfg'); if (d) _top_cfg = JSON.parse(d); } catch(e) {}
    if (!_top_cfg.server && !_top_cfg.m3u) _top_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _top_save() { providerSetItem('cfg', JSON.stringify(_top_cfg)); }
...
function getChanelsArray(cb) {
    _top_load();
    if (_top_cfg.server && _top_cfg.user && _top_cfg.pass) _top_xtream(cb);
    else if (_top_cfg.m3u) _top_m3u(cb);
    else { alert(_('Configure Top-Tv in Settings -> Provider Settings')); cb(); }
}
function _top_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _top_cfg.m3u, timeout: 15e3,
        success: function(d) { _top_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _top_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _top_parseM3U(d, cb); },
...
function _top_xtream(cb) {
    ...
    var api = _top_cfg.server + '/player_api.php?username=' + encodeURIComponent(_top_cfg.user) + '&password=' + encodeURIComponent(_top_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _top_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _top_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _top_cfg.server + '/live/' + encodeURIComponent(_top_cfg.user) + '/' + encodeURIComponent(_top_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _top_cfg.m3u = _top_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_top_cfg.user) + '&password=' + encodeURIComponent(_top_cfg.pass) + '&type=m3u_plus&output=ts';
        _top_m3u(cb);
    });
}
function duneAddSettings(e) {
    _top_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('Top-Tv settings'));
    popupActions.splice(e, 1, _top_edit);
    var idx = popupActions.indexOf(_top_edit);
    if (idx > -1) {
        var lbl = _('Top-Tv settings');
        if (_top_cfg.server && _top_cfg.user) lbl += ': ' + _top_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _top_cfg.user + ')';
        else if (_top_cfg.m3u) lbl += ': ' + _top_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _top_edit() {
    selIndex = 0; _top_load();
    var srv = _top_cfg.server, usr = _top_cfg.user, pwd = _top_cfg.pass, m3u = _top_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _top_cfg.server = srv; _top_cfg.user = usr; _top_cfg.pass = pwd; _top_cfg.m3u = m3u; _top_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('Top-Tv');
