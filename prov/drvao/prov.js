...
function _drvao_load() {
    try { var d = providerGetItem('cfg'); if (d) _drvao_cfg = JSON.parse(d); } catch(e) {}
    if (!_drvao_cfg.server && !_drvao_cfg.m3u) _drvao_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _drvao_save() { providerSetItem('cfg', JSON.stringify(_drvao_cfg)); }
...
function getChanelsArray(cb) {
    _drvao_load();
    if (_drvao_cfg.server && _drvao_cfg.user && _drvao_cfg.pass) _drvao_xtream(cb);
    else if (_drvao_cfg.m3u) _drvao_m3u(cb);
    else { alert(_('Configure DRVAO in Settings -> Provider Settings')); cb(); }
}
function _drvao_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _drvao_cfg.m3u, timeout: 15e3,
        success: function(d) { _drvao_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _drvao_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _drvao_parseM3U(d, cb); },
...
function _drvao_xtream(cb) {
    ...
    var api = _drvao_cfg.server + '/player_api.php?username=' + encodeURIComponent(_drvao_cfg.user) + '&password=' + encodeURIComponent(_drvao_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _drvao_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _drvao_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _drvao_cfg.server + '/live/' + encodeURIComponent(_drvao_cfg.user) + '/' + encodeURIComponent(_drvao_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _drvao_cfg.m3u = _drvao_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_drvao_cfg.user) + '&password=' + encodeURIComponent(_drvao_cfg.pass) + '&type=m3u_plus&output=ts';
        _drvao_m3u(cb);
    });
}
function duneAddSettings(e) {
    _drvao_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('DRVAO settings'));
    popupActions.splice(e, 1, _drvao_edit);
    var idx = popupActions.indexOf(_drvao_edit);
    if (idx > -1) {
        var lbl = _('DRVAO settings');
        if (_drvao_cfg.server && _drvao_cfg.user) lbl += ': ' + _drvao_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _drvao_cfg.user + ')';
        else if (_drvao_cfg.m3u) lbl += ': ' + _drvao_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _drvao_edit() {
    selIndex = 0; _drvao_load();
    var srv = _drvao_cfg.server, usr = _drvao_cfg.user, pwd = _drvao_cfg.pass, m3u = _drvao_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _drvao_cfg.server = srv; _drvao_cfg.user = usr; _drvao_cfg.pass = pwd; _drvao_cfg.m3u = m3u; _drvao_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('DRVAO');
