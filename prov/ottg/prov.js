...
var _ottg_cfg = { server: '', user: '', pass: '', m3u: '' };
function _ottg_load() {
    try { var d = providerGetItem('cfg'); if (d) _ottg_cfg = JSON.parse(d); } catch(e) {}
    if (!_ottg_cfg.server && !_ottg_cfg.m3u) _ottg_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _ottg_save() { providerSetItem('cfg', JSON.stringify(_ottg_cfg)); }
...
function getChanelsArray(cb) {
    _ottg_load();
    if (_ottg_cfg.server && _ottg_cfg.user && _ottg_cfg.pass) _ottg_xtream(cb);
    else if (_ottg_cfg.m3u) _ottg_m3u(cb);
    else { alert(_('Configure GlanzTV in Settings -> Provider Settings')); cb(); }
}
function _ottg_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _ottg_cfg.m3u, timeout: 15e3,
        success: function(d) { _ottg_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _ottg_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _ottg_parseM3U(d, cb); },
...
function _ottg_xtream(cb) {
    ...
    var api = _ottg_cfg.server + '/player_api.php?username=' + encodeURIComponent(_ottg_cfg.user) + '&password=' + encodeURIComponent(_ottg_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _ottg_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _ottg_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _ottg_cfg.server + '/live/' + encodeURIComponent(_ottg_cfg.user) + '/' + encodeURIComponent(_ottg_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _ottg_cfg.m3u = _ottg_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_ottg_cfg.user) + '&password=' + encodeURIComponent(_ottg_cfg.pass) + '&type=m3u_plus&output=ts';
        _ottg_m3u(cb);
    });
}
function duneAddSettings(e) {
    _ottg_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('GlanzTV settings'));
    popupActions.splice(e, 1, _ottg_edit);
    var idx = popupActions.indexOf(_ottg_edit);
    if (idx > -1) {
        var lbl = _('GlanzTV settings');
        if (_ottg_cfg.server && _ottg_cfg.user) lbl += ': ' + _ottg_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _ottg_cfg.user + ')';
        else if (_ottg_cfg.m3u) lbl += ': ' + _ottg_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _ottg_edit() {
    selIndex = 0; _ottg_load();
    var srv = _ottg_cfg.server, usr = _ottg_cfg.user, pwd = _ottg_cfg.pass, m3u = _ottg_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _ottg_cfg.server = srv; _ottg_cfg.user = usr; _ottg_cfg.pass = pwd; _ottg_cfg.m3u = m3u; _ottg_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('GlanzTV');
