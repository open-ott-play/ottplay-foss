...
var _russkoetv_cfg = { server: '', user: '', pass: '', m3u: '' };
function _russkoetv_load() {
    try { var d = providerGetItem('cfg'); if (d) _russkoetv_cfg = JSON.parse(d); } catch(e) {}
    if (!_russkoetv_cfg.server && !_russkoetv_cfg.m3u) _russkoetv_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _russkoetv_save() { providerSetItem('cfg', JSON.stringify(_russkoetv_cfg)); }
...
function getChanelsArray(cb) {
    _russkoetv_load();
    if (_russkoetv_cfg.server && _russkoetv_cfg.user && _russkoetv_cfg.pass) _russkoetv_xtream(cb);
    else if (_russkoetv_cfg.m3u) _russkoetv_m3u(cb);
    else { alert(_('Configure RUSSKOETV in Settings -> Provider Settings')); cb(); }
}
function _russkoetv_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _russkoetv_cfg.m3u, timeout: 15e3,
        success: function(d) { _russkoetv_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _russkoetv_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _russkoetv_parseM3U(d, cb); },
...
function _russkoetv_xtream(cb) {
    ...
    var api = _russkoetv_cfg.server + '/player_api.php?username=' + encodeURIComponent(_russkoetv_cfg.user) + '&password=' + encodeURIComponent(_russkoetv_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _russkoetv_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _russkoetv_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _russkoetv_cfg.server + '/live/' + encodeURIComponent(_russkoetv_cfg.user) + '/' + encodeURIComponent(_russkoetv_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _russkoetv_cfg.m3u = _russkoetv_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_russkoetv_cfg.user) + '&password=' + encodeURIComponent(_russkoetv_cfg.pass) + '&type=m3u_plus&output=ts';
        _russkoetv_m3u(cb);
    });
}
function duneAddSettings(e) {
    _russkoetv_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('RUSSKOETV settings'));
    popupActions.splice(e, 1, _russkoetv_edit);
    var idx = popupActions.indexOf(_russkoetv_edit);
    if (idx > -1) {
        var lbl = _('RUSSKOETV settings');
        if (_russkoetv_cfg.server && _russkoetv_cfg.user) lbl += ': ' + _russkoetv_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _russkoetv_cfg.user + ')';
        else if (_russkoetv_cfg.m3u) lbl += ': ' + _russkoetv_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _russkoetv_edit() {
    selIndex = 0; _russkoetv_load();
    var srv = _russkoetv_cfg.server, usr = _russkoetv_cfg.user, pwd = _russkoetv_cfg.pass, m3u = _russkoetv_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _russkoetv_cfg.server = srv; _russkoetv_cfg.user = usr; _russkoetv_cfg.pass = pwd; _russkoetv_cfg.m3u = m3u; _russkoetv_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('RUSSKOETV');
