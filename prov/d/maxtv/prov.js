...
var _d_maxtv_cfg = { m3u: "", pass: "", server: "", user: "" };
function _d_maxtv_load() {
    try {
        var d = providerGetItem("cfg");
        if (d) _d_maxtv_cfg = JSON.parse(d);
    } catch (e) {}
    if (!(_d_maxtv_cfg.server || _d_maxtv_cfg.m3u))
        _d_maxtv_cfg = { m3u: "", pass: "", server: "", user: "" };
}
function _d_maxtv_save() {
    providerSetItem("cfg", JSON.stringify(_d_maxtv_cfg));
}
...
function getChanelsArray(cb) {
    _d_maxtv_load();
    if (_d_maxtv_cfg.server && _d_maxtv_cfg.user && _d_maxtv_cfg.pass)
        _d_maxtv_xtream(cb);
    else if (_d_maxtv_cfg.m3u) _d_maxtv_m3u(cb);
    else {
        alert(_("Configure MaxTV in Settings -> Provider Settings"));
        cb();
    }
}
...
function _d_maxtv_xtream(cb) {
    ...
    var api =
        ...
        encodeURIComponent(_d_maxtv_cfg.user) +
        "&password=" +
        encodeURIComponent(_d_maxtv_cfg.pass);
...
function duneAddSettings(e) {
    ...
    popupDetail.splice(e, 1, _("MaxTV settings"));
    popupActions.splice(e, 1, _d_maxtv_edit);
    var idx = popupActions.indexOf(_d_maxtv_edit);
    if (idx > -1) {
        var lbl = _("MaxTV settings");
        if (_d_maxtv_cfg.server && _d_maxtv_cfg.user)
            lbl +=
                ": " +
                _d_maxtv_cfg.server.replace(/^https?:\/\//, "").split("/")[0] +
                " (" +
                _d_maxtv_cfg.user +
                ")";
        else if (_d_maxtv_cfg.m3u)
            lbl += ": " + _d_maxtv_cfg.m3u.substr(0, 40) + "...";
        popupArray[idx] = lbl;
    }
}
            ...
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 3:
                        ...
                        return true;
                    case 5:
                        _d_maxtv_cfg.server = srv;
                        _d_maxtv_cfg.user = usr;
                        _d_maxtv_cfg.pass = pwd;
                        _d_maxtv_cfg.m3u = m3u;
                        _d_maxtv_save();
                        duneAddSettings(0);
                        loadChannels();
                        return true;
