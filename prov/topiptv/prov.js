...
function _topiptv_load() {
    ...
    if (!(_topiptv_cfg.server || _topiptv_cfg.m3u))
        _topiptv_cfg = { server: "", user: "", pass: "", m3u: "" };
}
...
function getChanelsArray(cb) {
    _topiptv_load();
    if (_topiptv_cfg.server && _topiptv_cfg.user && _topiptv_cfg.pass)
        _topiptv_xtream(cb);
...
function _topiptv_m3u(cb) {
    ...
    $.ajax({
        url: _topiptv_cfg.m3u,
        timeout: 15e3,
        success: function (d) {
            _topiptv_parseM3U(d, cb);
        },
        error: function () {
            $.ajax({
                url: host + "/m3u/cp.php",
                data: { url: "@" + _topiptv_cfg.m3u },
                method: "post",
                dataType: "text",
                timeout: 15e3,
                success: function (d) {
...
function _topiptv_xtream(cb) {
    ...
    var api =
        ...
        encodeURIComponent(_topiptv_cfg.user) +
        "&password=" +
        encodeURIComponent(_topiptv_cfg.pass);
