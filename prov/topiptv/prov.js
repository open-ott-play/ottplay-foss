...
var _topiptv_cfg = { server: '', user: '', pass: '', m3u: '' };
function _topiptv_load() {
    try { var d = providerGetItem('cfg'); if (d) _topiptv_cfg = JSON.parse(d); } catch(e) {}
    if (!_topiptv_cfg.server && !_topiptv_cfg.m3u) _topiptv_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _topiptv_save() { providerSetItem('cfg', JSON.stringify(_topiptv_cfg)); }
...
function getChanelsArray(cb) {
    _topiptv_load();
    if (_topiptv_cfg.server && _topiptv_cfg.user && _topiptv_cfg.pass) _topiptv_xtream(cb);
    else if (_topiptv_cfg.m3u) _topiptv_m3u(cb);
    else { alert(_('Configure TOP-IPTV in Settings -> Provider Settings')); cb(); }
}
function _topiptv_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _topiptv_cfg.m3u, timeout: 15e3,
        success: function(d) { _topiptv_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _topiptv_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _topiptv_parseM3U(d, cb); },
...
function _topiptv_xtream(cb) {
    ...
    var api = _topiptv_cfg.server + '/player_api.php?username=' + encodeURIComponent(_topiptv_cfg.user) + '&password=' + encodeURIComponent(_topiptv_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _topiptv_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _topiptv_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _topiptv_cfg.server + '/live/' + encodeURIComponent(_topiptv_cfg.user) + '/' + encodeURIComponent(_topiptv_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _topiptv_cfg.m3u = _topiptv_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_topiptv_cfg.user) + '&password=' + encodeURIComponent(_topiptv_cfg.pass) + '&type=m3u_plus&output=ts';
        _topiptv_m3u(cb);
    });
}
function duneAddSettings(e) {
    _topiptv_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('TOP-IPTV settings'));
    popupActions.splice(e, 1, _topiptv_edit);
    var idx = popupActions.indexOf(_topiptv_edit);
    if (idx > -1) {
        var lbl = _('TOP-IPTV settings');
        if (_topiptv_cfg.server && _topiptv_cfg.user) lbl += ': ' + _topiptv_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _topiptv_cfg.user + ')';
        else if (_topiptv_cfg.m3u) lbl += ': ' + _topiptv_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _topiptv_edit() {
    selIndex = 0; _topiptv_load();
    var srv = _topiptv_cfg.server, usr = _topiptv_cfg.user, pwd = _topiptv_cfg.pass, m3u = _topiptv_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _topiptv_cfg.server = srv; _topiptv_cfg.user = usr; _topiptv_cfg.pass = pwd; _topiptv_cfg.m3u = m3u; _topiptv_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('TOP-IPTV');
