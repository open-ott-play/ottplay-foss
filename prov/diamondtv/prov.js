...
var _diamondtv_cfg = { server: '', user: '', pass: '', m3u: '' };
function _diamondtv_load() {
    try { var d = providerGetItem('cfg'); if (d) _diamondtv_cfg = JSON.parse(d); } catch(e) {}
    if (!_diamondtv_cfg.server && !_diamondtv_cfg.m3u) _diamondtv_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _diamondtv_save() { providerSetItem('cfg', JSON.stringify(_diamondtv_cfg)); }
...
function getChanelsArray(cb) {
    _diamondtv_load();
    if (_diamondtv_cfg.server && _diamondtv_cfg.user && _diamondtv_cfg.pass) _diamondtv_xtream(cb);
    else if (_diamondtv_cfg.m3u) _diamondtv_m3u(cb);
    else { alert(_('Configure Diamond TV in Settings -> Provider Settings')); cb(); }
}
function _diamondtv_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _diamondtv_cfg.m3u, timeout: 15e3,
        success: function(d) { _diamondtv_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _diamondtv_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _diamondtv_parseM3U(d, cb); },
...
function _diamondtv_xtream(cb) {
    ...
    var api = _diamondtv_cfg.server + '/player_api.php?username=' + encodeURIComponent(_diamondtv_cfg.user) + '&password=' + encodeURIComponent(_diamondtv_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _diamondtv_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _diamondtv_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _diamondtv_cfg.server + '/live/' + encodeURIComponent(_diamondtv_cfg.user) + '/' + encodeURIComponent(_diamondtv_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _diamondtv_cfg.m3u = _diamondtv_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_diamondtv_cfg.user) + '&password=' + encodeURIComponent(_diamondtv_cfg.pass) + '&type=m3u_plus&output=ts';
        _diamondtv_m3u(cb);
    });
}
function duneAddSettings(e) {
    _diamondtv_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('Diamond TV settings'));
    popupActions.splice(e, 1, _diamondtv_edit);
    var idx = popupActions.indexOf(_diamondtv_edit);
    if (idx > -1) {
        var lbl = _('Diamond TV settings');
        if (_diamondtv_cfg.server && _diamondtv_cfg.user) lbl += ': ' + _diamondtv_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _diamondtv_cfg.user + ')';
        else if (_diamondtv_cfg.m3u) lbl += ': ' + _diamondtv_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _diamondtv_edit() {
    selIndex = 0; _diamondtv_load();
    var srv = _diamondtv_cfg.server, usr = _diamondtv_cfg.user, pwd = _diamondtv_cfg.pass, m3u = _diamondtv_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _diamondtv_cfg.server = srv; _diamondtv_cfg.user = usr; _diamondtv_cfg.pass = pwd; _diamondtv_cfg.m3u = m3u; _diamondtv_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('Diamond TV');
