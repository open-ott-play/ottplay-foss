...
function _diamondtv_load() {
    ...
    if (!(_diamondtv_cfg.server || _diamondtv_cfg.m3u))
        _diamondtv_cfg = { server: "", user: "", pass: "", m3u: "" };
}
...
function getChanelsArray(cb) {
    _diamondtv_load();
    if (_diamondtv_cfg.server && _diamondtv_cfg.user && _diamondtv_cfg.pass)
        _diamondtv_xtream(cb);
...
function _diamondtv_m3u(cb) {
    ...
    $.ajax({
        url: _diamondtv_cfg.m3u,
        timeout: 15e3,
        success: function (d) {
            _diamondtv_parseM3U(d, cb);
        },
        error: function () {
            $.ajax({
                url: host + "/m3u/cp.php",
                data: { url: "@" + _diamondtv_cfg.m3u },
                method: "post",
                dataType: "text",
                timeout: 15e3,
                success: function (d) {
...
function _diamondtv_xtream(cb) {
    ...
    var api =
        ...
        encodeURIComponent(_diamondtv_cfg.user) +
        "&password=" +
        encodeURIComponent(_diamondtv_cfg.pass);
