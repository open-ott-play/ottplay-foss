...
var _ultifl1x_cfg = { server: '', user: '', pass: '', m3u: '' };
function _ultifl1x_load() {
    try { var d = providerGetItem('cfg'); if (d) _ultifl1x_cfg = JSON.parse(d); } catch(e) {}
    if (!_ultifl1x_cfg.server && !_ultifl1x_cfg.m3u) _ultifl1x_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _ultifl1x_save() { providerSetItem('cfg', JSON.stringify(_ultifl1x_cfg)); }
...
function getChanelsArray(cb) {
    _ultifl1x_load();
    if (_ultifl1x_cfg.server && _ultifl1x_cfg.user && _ultifl1x_cfg.pass) _ultifl1x_xtream(cb);
    else if (_ultifl1x_cfg.m3u) _ultifl1x_m3u(cb);
    else { alert(_('Configure ULTIFL1X in Settings -> Provider Settings')); cb(); }
}
function _ultifl1x_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _ultifl1x_cfg.m3u, timeout: 15e3,
        success: function(d) { _ultifl1x_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _ultifl1x_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _ultifl1x_parseM3U(d, cb); },
...
function _ultifl1x_xtream(cb) {
    ...
    var api = _ultifl1x_cfg.server + '/player_api.php?username=' + encodeURIComponent(_ultifl1x_cfg.user) + '&password=' + encodeURIComponent(_ultifl1x_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _ultifl1x_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _ultifl1x_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _ultifl1x_cfg.server + '/live/' + encodeURIComponent(_ultifl1x_cfg.user) + '/' + encodeURIComponent(_ultifl1x_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _ultifl1x_cfg.m3u = _ultifl1x_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_ultifl1x_cfg.user) + '&password=' + encodeURIComponent(_ultifl1x_cfg.pass) + '&type=m3u_plus&output=ts';
        _ultifl1x_m3u(cb);
    });
}
function duneAddSettings(e) {
    _ultifl1x_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('ULTIFL1X settings'));
    popupActions.splice(e, 1, _ultifl1x_edit);
    var idx = popupActions.indexOf(_ultifl1x_edit);
    if (idx > -1) {
        var lbl = _('ULTIFL1X settings');
        if (_ultifl1x_cfg.server && _ultifl1x_cfg.user) lbl += ': ' + _ultifl1x_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _ultifl1x_cfg.user + ')';
        else if (_ultifl1x_cfg.m3u) lbl += ': ' + _ultifl1x_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _ultifl1x_edit() {
    selIndex = 0; _ultifl1x_load();
    var srv = _ultifl1x_cfg.server, usr = _ultifl1x_cfg.user, pwd = _ultifl1x_cfg.pass, m3u = _ultifl1x_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _ultifl1x_cfg.server = srv; _ultifl1x_cfg.user = usr; _ultifl1x_cfg.pass = pwd; _ultifl1x_cfg.m3u = m3u; _ultifl1x_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('ULTIFL1X');
