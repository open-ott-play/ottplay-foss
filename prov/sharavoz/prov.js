...
var _sharavoz_cfg = { server: '', user: '', pass: '', m3u: '' };
function _sharavoz_load() {
    try { var d = providerGetItem('cfg'); if (d) _sharavoz_cfg = JSON.parse(d); } catch(e) {}
    if (!_sharavoz_cfg.server && !_sharavoz_cfg.m3u) _sharavoz_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _sharavoz_save() { providerSetItem('cfg', JSON.stringify(_sharavoz_cfg)); }
...
function getChanelsArray(cb) {
    _sharavoz_load();
    if (_sharavoz_cfg.server && _sharavoz_cfg.user && _sharavoz_cfg.pass) _sharavoz_xtream(cb);
    else if (_sharavoz_cfg.m3u) _sharavoz_m3u(cb);
    else { alert(_('Configure Шаравоз in Settings -> Provider Settings')); cb(); }
}
function _sharavoz_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _sharavoz_cfg.m3u, timeout: 15e3,
        success: function(d) { _sharavoz_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _sharavoz_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _sharavoz_parseM3U(d, cb); },
...
function _sharavoz_xtream(cb) {
    ...
    var api = _sharavoz_cfg.server + '/player_api.php?username=' + encodeURIComponent(_sharavoz_cfg.user) + '&password=' + encodeURIComponent(_sharavoz_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _sharavoz_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _sharavoz_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _sharavoz_cfg.server + '/live/' + encodeURIComponent(_sharavoz_cfg.user) + '/' + encodeURIComponent(_sharavoz_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _sharavoz_cfg.m3u = _sharavoz_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_sharavoz_cfg.user) + '&password=' + encodeURIComponent(_sharavoz_cfg.pass) + '&type=m3u_plus&output=ts';
        _sharavoz_m3u(cb);
    });
}
function duneAddSettings(e) {
    _sharavoz_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('Шаравоз settings'));
    popupActions.splice(e, 1, _sharavoz_edit);
    var idx = popupActions.indexOf(_sharavoz_edit);
    if (idx > -1) {
        var lbl = _('Шаравоз settings');
        if (_sharavoz_cfg.server && _sharavoz_cfg.user) lbl += ': ' + _sharavoz_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _sharavoz_cfg.user + ')';
        else if (_sharavoz_cfg.m3u) lbl += ': ' + _sharavoz_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _sharavoz_edit() {
    selIndex = 0; _sharavoz_load();
    var srv = _sharavoz_cfg.server, usr = _sharavoz_cfg.user, pwd = _sharavoz_cfg.pass, m3u = _sharavoz_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _sharavoz_cfg.server = srv; _sharavoz_cfg.user = usr; _sharavoz_cfg.pass = pwd; _sharavoz_cfg.m3u = m3u; _sharavoz_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('Шаравоз');
