...
function _sharavoz_load() {
    ...
    if (!(_sharavoz_cfg.server || _sharavoz_cfg.m3u))
        _sharavoz_cfg = { server: "", user: "", pass: "", m3u: "" };
}
...
function getChanelsArray(cb) {
    _sharavoz_load();
    if (_sharavoz_cfg.server && _sharavoz_cfg.user && _sharavoz_cfg.pass)
        _sharavoz_xtream(cb);
...
function _sharavoz_m3u(cb) {
    ...
    $.ajax({
        url: _sharavoz_cfg.m3u,
        timeout: 15e3,
        success: function (d) {
            _sharavoz_parseM3U(d, cb);
        },
        error: function () {
            $.ajax({
                url: host + "/m3u/cp.php",
                data: { url: "@" + _sharavoz_cfg.m3u },
                method: "post",
                dataType: "text",
                timeout: 15e3,
                success: function (d) {
...
function _sharavoz_xtream(cb) {
    ...
    var api =
        _sharavoz_cfg.server +
        "/player_api.php?username=" +
        encodeURIComponent(_sharavoz_cfg.user) +
        "&password=" +
        encodeURIComponent(_sharavoz_cfg.pass);
