...
var _shara_tv_cfg = { server: '', user: '', pass: '', m3u: '' };
function _shara_tv_load() {
    try { var d = providerGetItem('cfg'); if (d) _shara_tv_cfg = JSON.parse(d); } catch(e) {}
    if (!_shara_tv_cfg.server && !_shara_tv_cfg.m3u) _shara_tv_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _shara_tv_save() { providerSetItem('cfg', JSON.stringify(_shara_tv_cfg)); }
...
function getChanelsArray(cb) {
    _shara_tv_load();
    if (_shara_tv_cfg.server && _shara_tv_cfg.user && _shara_tv_cfg.pass) _shara_tv_xtream(cb);
    else if (_shara_tv_cfg.m3u) _shara_tv_m3u(cb);
    else { alert(_('Configure Shara-TV in Settings -> Provider Settings')); cb(); }
}
function _shara_tv_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _shara_tv_cfg.m3u, timeout: 15e3,
        success: function(d) { _shara_tv_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _shara_tv_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _shara_tv_parseM3U(d, cb); },
...
function _shara_tv_xtream(cb) {
    ...
    var api = _shara_tv_cfg.server + '/player_api.php?username=' + encodeURIComponent(_shara_tv_cfg.user) + '&password=' + encodeURIComponent(_shara_tv_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _shara_tv_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _shara_tv_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _shara_tv_cfg.server + '/live/' + encodeURIComponent(_shara_tv_cfg.user) + '/' + encodeURIComponent(_shara_tv_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _shara_tv_cfg.m3u = _shara_tv_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_shara_tv_cfg.user) + '&password=' + encodeURIComponent(_shara_tv_cfg.pass) + '&type=m3u_plus&output=ts';
        _shara_tv_m3u(cb);
    });
}
function duneAddSettings(e) {
    _shara_tv_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('Shara-TV settings'));
    popupActions.splice(e, 1, _shara_tv_edit);
    var idx = popupActions.indexOf(_shara_tv_edit);
    if (idx > -1) {
        var lbl = _('Shara-TV settings');
        if (_shara_tv_cfg.server && _shara_tv_cfg.user) lbl += ': ' + _shara_tv_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _shara_tv_cfg.user + ')';
        else if (_shara_tv_cfg.m3u) lbl += ': ' + _shara_tv_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _shara_tv_edit() {
    selIndex = 0; _shara_tv_load();
    var srv = _shara_tv_cfg.server, usr = _shara_tv_cfg.user, pwd = _shara_tv_cfg.pass, m3u = _shara_tv_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _shara_tv_cfg.server = srv; _shara_tv_cfg.user = usr; _shara_tv_cfg.pass = pwd; _shara_tv_cfg.m3u = m3u; _shara_tv_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('Shara-TV');
