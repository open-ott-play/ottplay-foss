...
var _rd_cfg = { server: '', user: '', pass: '', m3u: '' };
function _rd_load() {
    try { var d = providerGetItem('cfg'); if (d) _rd_cfg = JSON.parse(d); } catch(e) {}
    if (!_rd_cfg.server && !_rd_cfg.m3u) _rd_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _rd_save() { providerSetItem('cfg', JSON.stringify(_rd_cfg)); }
...
function getChanelsArray(cb) {
    _rd_load();
    if (_rd_cfg.server && _rd_cfg.user && _rd_cfg.pass) _rd_xtream(cb);
    else if (_rd_cfg.m3u) _rd_m3u(cb);
    else { alert(_('Configure RD in Settings -> Provider Settings')); cb(); }
}
function _rd_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _rd_cfg.m3u, timeout: 15e3,
        success: function(d) { _rd_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _rd_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _rd_parseM3U(d, cb); },
...
function _rd_xtream(cb) {
    ...
    var api = _rd_cfg.server + '/player_api.php?username=' + encodeURIComponent(_rd_cfg.user) + '&password=' + encodeURIComponent(_rd_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _rd_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _rd_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _rd_cfg.server + '/live/' + encodeURIComponent(_rd_cfg.user) + '/' + encodeURIComponent(_rd_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _rd_cfg.m3u = _rd_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_rd_cfg.user) + '&password=' + encodeURIComponent(_rd_cfg.pass) + '&type=m3u_plus&output=ts';
        _rd_m3u(cb);
    });
}
function duneAddSettings(e) {
    _rd_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('RD settings'));
    popupActions.splice(e, 1, _rd_edit);
    var idx = popupActions.indexOf(_rd_edit);
    if (idx > -1) {
        var lbl = _('RD settings');
        if (_rd_cfg.server && _rd_cfg.user) lbl += ': ' + _rd_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _rd_cfg.user + ')';
...
function _rd_edit() {
    selIndex = 0; _rd_load();
    var srv = _rd_cfg.server, usr = _rd_cfg.user, pwd = _rd_cfg.pass, m3u = _rd_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _rd_cfg.server = srv; _rd_cfg.user = usr; _rd_cfg.pass = pwd; _rd_cfg.m3u = m3u; _rd_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('RD');
