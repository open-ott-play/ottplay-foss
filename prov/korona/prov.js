...
function _korona_load() {
    ...
    if (!(_korona_cfg.server || _korona_cfg.m3u))
        _korona_cfg = { server: "", user: "", pass: "", m3u: "" };
}
...
function getChanelsArray(cb) {
    _korona_load();
    if (_korona_cfg.server && _korona_cfg.user && _korona_cfg.pass)
        _korona_xtream(cb);
...
function _korona_m3u(cb) {
    ...
    $.ajax({
        ...
        timeout: 15e3,
        success: function (d) {
            _korona_parseM3U(d, cb);
        },
        error: function () {
            $.ajax({
                url: host + "/m3u/cp.php",
                data: { url: "@" + _korona_cfg.m3u },
                method: "post",
                dataType: "text",
                timeout: 15e3,
                success: function (d) {
...
function _korona_xtream(cb) {
    ...
    var api =
        ...
        encodeURIComponent(_korona_cfg.user) +
        "&password=" +
        encodeURIComponent(_korona_cfg.pass);
