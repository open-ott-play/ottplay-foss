...
var _korona_cfg = { server: '', user: '', pass: '', m3u: '' };
function _korona_load() {
    try { var d = providerGetItem('cfg'); if (d) _korona_cfg = JSON.parse(d); } catch(e) {}
    if (!_korona_cfg.server && !_korona_cfg.m3u) _korona_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _korona_save() { providerSetItem('cfg', JSON.stringify(_korona_cfg)); }
...
function getChanelsArray(cb) {
    _korona_load();
    if (_korona_cfg.server && _korona_cfg.user && _korona_cfg.pass) _korona_xtream(cb);
    else if (_korona_cfg.m3u) _korona_m3u(cb);
    else { alert(_('Configure KORONA TV in Settings -> Provider Settings')); cb(); }
}
function _korona_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _korona_cfg.m3u, timeout: 15e3,
        success: function(d) { _korona_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _korona_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _korona_parseM3U(d, cb); },
...
function _korona_xtream(cb) {
    ...
    var api = _korona_cfg.server + '/player_api.php?username=' + encodeURIComponent(_korona_cfg.user) + '&password=' + encodeURIComponent(_korona_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _korona_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _korona_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _korona_cfg.server + '/live/' + encodeURIComponent(_korona_cfg.user) + '/' + encodeURIComponent(_korona_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _korona_cfg.m3u = _korona_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_korona_cfg.user) + '&password=' + encodeURIComponent(_korona_cfg.pass) + '&type=m3u_plus&output=ts';
        _korona_m3u(cb);
    });
}
function duneAddSettings(e) {
    _korona_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('KORONA TV settings'));
    popupActions.splice(e, 1, _korona_edit);
    var idx = popupActions.indexOf(_korona_edit);
    if (idx > -1) {
        var lbl = _('KORONA TV settings');
        if (_korona_cfg.server && _korona_cfg.user) lbl += ': ' + _korona_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _korona_cfg.user + ')';
        else if (_korona_cfg.m3u) lbl += ': ' + _korona_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _korona_edit() {
    selIndex = 0; _korona_load();
    var srv = _korona_cfg.server, usr = _korona_cfg.user, pwd = _korona_cfg.pass, m3u = _korona_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _korona_cfg.server = srv; _korona_cfg.user = usr; _korona_cfg.pass = pwd; _korona_cfg.m3u = m3u; _korona_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('KORONA TV');
