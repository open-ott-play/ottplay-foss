...
var _ottprime_cfg = { server: '', user: '', pass: '', m3u: '' };
function _ottprime_load() {
    try { var d = providerGetItem('cfg'); if (d) _ottprime_cfg = JSON.parse(d); } catch(e) {}
    if (!_ottprime_cfg.server && !_ottprime_cfg.m3u) _ottprime_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _ottprime_save() { providerSetItem('cfg', JSON.stringify(_ottprime_cfg)); }
...
function getChanelsArray(cb) {
    _ottprime_load();
    if (_ottprime_cfg.server && _ottprime_cfg.user && _ottprime_cfg.pass) _ottprime_xtream(cb);
    else if (_ottprime_cfg.m3u) _ottprime_m3u(cb);
    else { alert(_('Configure OTT Prime ONLINE in Settings -> Provider Settings')); cb(); }
}
function _ottprime_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _ottprime_cfg.m3u, timeout: 15e3,
        success: function(d) { _ottprime_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _ottprime_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _ottprime_parseM3U(d, cb); },
...
function _ottprime_xtream(cb) {
    ...
    var api = _ottprime_cfg.server + '/player_api.php?username=' + encodeURIComponent(_ottprime_cfg.user) + '&password=' + encodeURIComponent(_ottprime_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _ottprime_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _ottprime_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _ottprime_cfg.server + '/live/' + encodeURIComponent(_ottprime_cfg.user) + '/' + encodeURIComponent(_ottprime_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _ottprime_cfg.m3u = _ottprime_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_ottprime_cfg.user) + '&password=' + encodeURIComponent(_ottprime_cfg.pass) + '&type=m3u_plus&output=ts';
        _ottprime_m3u(cb);
    });
}
function duneAddSettings(e) {
    _ottprime_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('OTT Prime ONLINE settings'));
    popupActions.splice(e, 1, _ottprime_edit);
    var idx = popupActions.indexOf(_ottprime_edit);
    if (idx > -1) {
        var lbl = _('OTT Prime ONLINE settings');
        if (_ottprime_cfg.server && _ottprime_cfg.user) lbl += ': ' + _ottprime_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _ottprime_cfg.user + ')';
        else if (_ottprime_cfg.m3u) lbl += ': ' + _ottprime_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _ottprime_edit() {
    selIndex = 0; _ottprime_load();
    var srv = _ottprime_cfg.server, usr = _ottprime_cfg.user, pwd = _ottprime_cfg.pass, m3u = _ottprime_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _ottprime_cfg.server = srv; _ottprime_cfg.user = usr; _ottprime_cfg.pass = pwd; _ottprime_cfg.m3u = m3u; _ottprime_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('OTT Prime ONLINE');
