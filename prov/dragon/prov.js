...
var _dragon_cfg = { server: '', user: '', pass: '', m3u: '' };
function _dragon_load() {
    try { var d = providerGetItem('cfg'); if (d) _dragon_cfg = JSON.parse(d); } catch(e) {}
    if (!_dragon_cfg.server && !_dragon_cfg.m3u) _dragon_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _dragon_save() { providerSetItem('cfg', JSON.stringify(_dragon_cfg)); }
...
function getChanelsArray(cb) {
    _dragon_load();
    if (_dragon_cfg.server && _dragon_cfg.user && _dragon_cfg.pass) _dragon_xtream(cb);
    else if (_dragon_cfg.m3u) _dragon_m3u(cb);
    else { alert(_('Configure Dragon Media PRO in Settings -> Provider Settings')); cb(); }
}
function _dragon_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _dragon_cfg.m3u, timeout: 15e3,
        success: function(d) { _dragon_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _dragon_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _dragon_parseM3U(d, cb); },
...
function _dragon_xtream(cb) {
    ...
    var api = _dragon_cfg.server + '/player_api.php?username=' + encodeURIComponent(_dragon_cfg.user) + '&password=' + encodeURIComponent(_dragon_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _dragon_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _dragon_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _dragon_cfg.server + '/live/' + encodeURIComponent(_dragon_cfg.user) + '/' + encodeURIComponent(_dragon_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _dragon_cfg.m3u = _dragon_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_dragon_cfg.user) + '&password=' + encodeURIComponent(_dragon_cfg.pass) + '&type=m3u_plus&output=ts';
        _dragon_m3u(cb);
    });
}
function duneAddSettings(e) {
    _dragon_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('Dragon Media PRO settings'));
    popupActions.splice(e, 1, _dragon_edit);
    var idx = popupActions.indexOf(_dragon_edit);
    if (idx > -1) {
        var lbl = _('Dragon Media PRO settings');
        if (_dragon_cfg.server && _dragon_cfg.user) lbl += ': ' + _dragon_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _dragon_cfg.user + ')';
        else if (_dragon_cfg.m3u) lbl += ': ' + _dragon_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _dragon_edit() {
    selIndex = 0; _dragon_load();
    var srv = _dragon_cfg.server, usr = _dragon_cfg.user, pwd = _dragon_cfg.pass, m3u = _dragon_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _dragon_cfg.server = srv; _dragon_cfg.user = usr; _dragon_cfg.pass = pwd; _dragon_cfg.m3u = m3u; _dragon_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('Dragon Media PRO');
