...
var _prost_cfg = { server: '', user: '', pass: '', m3u: '' };
function _prost_load() {
    try { var d = providerGetItem('cfg'); if (d) _prost_cfg = JSON.parse(d); } catch(e) {}
    if (!_prost_cfg.server && !_prost_cfg.m3u) _prost_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _prost_save() { providerSetItem('cfg', JSON.stringify(_prost_cfg)); }
...
function getChanelsArray(cb) {
    _prost_load();
    if (_prost_cfg.server && _prost_cfg.user && _prost_cfg.pass) _prost_xtream(cb);
    else if (_prost_cfg.m3u) _prost_m3u(cb);
    else { alert(_('Configure PROST in Settings -> Provider Settings')); cb(); }
}
function _prost_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _prost_cfg.m3u, timeout: 15e3,
        success: function(d) { _prost_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _prost_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _prost_parseM3U(d, cb); },
...
function _prost_xtream(cb) {
    ...
    var api = _prost_cfg.server + '/player_api.php?username=' + encodeURIComponent(_prost_cfg.user) + '&password=' + encodeURIComponent(_prost_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _prost_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _prost_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _prost_cfg.server + '/live/' + encodeURIComponent(_prost_cfg.user) + '/' + encodeURIComponent(_prost_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _prost_cfg.m3u = _prost_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_prost_cfg.user) + '&password=' + encodeURIComponent(_prost_cfg.pass) + '&type=m3u_plus&output=ts';
        _prost_m3u(cb);
    });
}
function duneAddSettings(e) {
    _prost_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('PROST settings'));
    popupActions.splice(e, 1, _prost_edit);
    var idx = popupActions.indexOf(_prost_edit);
    if (idx > -1) {
        var lbl = _('PROST settings');
        if (_prost_cfg.server && _prost_cfg.user) lbl += ': ' + _prost_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _prost_cfg.user + ')';
        else if (_prost_cfg.m3u) lbl += ': ' + _prost_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _prost_edit() {
    selIndex = 0; _prost_load();
    var srv = _prost_cfg.server, usr = _prost_cfg.user, pwd = _prost_cfg.pass, m3u = _prost_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _prost_cfg.server = srv; _prost_cfg.user = usr; _prost_cfg.pass = pwd; _prost_cfg.m3u = m3u; _prost_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('PROST');
