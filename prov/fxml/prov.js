...
var _fxml_cfg = { server: '', user: '', pass: '', m3u: '' };
function _fxml_load() {
    try { var d = providerGetItem('cfg'); if (d) _fxml_cfg = JSON.parse(d); } catch(e) {}
    if (!_fxml_cfg.server && !_fxml_cfg.m3u) _fxml_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _fxml_save() { providerSetItem('cfg', JSON.stringify(_fxml_cfg)); }
...
function getChanelsArray(cb) {
    ...
    else { alert(_('Configure FXML in Settings -> Provider Settings')); cb(); }
}
function _fxml_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _fxml_cfg.m3u, timeout: 15e3,
        success: function(d) { _fxml_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _fxml_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _fxml_parseM3U(d, cb); },
...
function _fxml_xtream(cb) {
    ...
    var api = _fxml_cfg.server + '/player_api.php?username=' + encodeURIComponent(_fxml_cfg.user) + '&password=' + encodeURIComponent(_fxml_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _fxml_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _fxml_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _fxml_cfg.server + '/live/' + encodeURIComponent(_fxml_cfg.user) + '/' + encodeURIComponent(_fxml_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _fxml_cfg.m3u = _fxml_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_fxml_cfg.user) + '&password=' + encodeURIComponent(_fxml_cfg.pass) + '&type=m3u_plus&output=ts';
        _fxml_m3u(cb);
    });
}
function duneAddSettings(e) {
    _fxml_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('FXML settings'));
    popupActions.splice(e, 1, _fxml_edit);
    var idx = popupActions.indexOf(_fxml_edit);
    if (idx > -1) {
        var lbl = _('FXML settings');
        if (_fxml_cfg.server && _fxml_cfg.user) lbl += ': ' + _fxml_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _fxml_cfg.user + ')';
        else if (_fxml_cfg.m3u) lbl += ': ' + _fxml_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _fxml_edit() {
    selIndex = 0; _fxml_load();
    var srv = _fxml_cfg.server, usr = _fxml_cfg.user, pwd = _fxml_cfg.pass, m3u = _fxml_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _fxml_cfg.server = srv; _fxml_cfg.user = usr; _fxml_cfg.pass = pwd; _fxml_cfg.m3u = m3u; _fxml_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('FXML');
