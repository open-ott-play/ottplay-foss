...
var _kb_team_cfg = { server: '', user: '', pass: '', m3u: '' };
function _kb_team_load() {
    try { var d = providerGetItem('cfg'); if (d) _kb_team_cfg = JSON.parse(d); } catch(e) {}
    if (!_kb_team_cfg.server && !_kb_team_cfg.m3u) _kb_team_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _kb_team_save() { providerSetItem('cfg', JSON.stringify(_kb_team_cfg)); }
...
function getChanelsArray(cb) {
    _kb_team_load();
    if (_kb_team_cfg.server && _kb_team_cfg.user && _kb_team_cfg.pass) _kb_team_xtream(cb);
    else if (_kb_team_cfg.m3u) _kb_team_m3u(cb);
    else { alert(_('Configure KBC (Kinoboom) in Settings -> Provider Settings')); cb(); }
}
function _kb_team_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _kb_team_cfg.m3u, timeout: 15e3,
        success: function(d) { _kb_team_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _kb_team_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _kb_team_parseM3U(d, cb); },
...
function _kb_team_parseM3U(data, cb) {
...
function _kb_team_xtream(cb) {
    $(launch_id).append(_('Loading from API...'));
    var api = _kb_team_cfg.server + '/player_api.php?username=' + encodeURIComponent(_kb_team_cfg.user) + '&password=' + encodeURIComponent(_kb_team_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _kb_team_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _kb_team_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _kb_team_cfg.server + '/live/' + encodeURIComponent(_kb_team_cfg.user) + '/' + encodeURIComponent(_kb_team_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _kb_team_cfg.m3u = _kb_team_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_kb_team_cfg.user) + '&password=' + encodeURIComponent(_kb_team_cfg.pass) + '&type=m3u_plus&output=ts';
        _kb_team_m3u(cb);
    });
}
function duneAddSettings(e) {
    _kb_team_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('KBC (Kinoboom) settings'));
    popupActions.splice(e, 1, _kb_team_edit);
    var idx = popupActions.indexOf(_kb_team_edit);
    if (idx > -1) {
        var lbl = _('KBC (Kinoboom) settings');
        if (_kb_team_cfg.server && _kb_team_cfg.user) lbl += ': ' + _kb_team_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _kb_team_cfg.user + ')';
        else if (_kb_team_cfg.m3u) lbl += ': ' + _kb_team_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _kb_team_edit() {
    selIndex = 0; _kb_team_load();
    var srv = _kb_team_cfg.server, usr = _kb_team_cfg.user, pwd = _kb_team_cfg.pass, m3u = _kb_team_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _kb_team_cfg.server = srv; _kb_team_cfg.user = usr; _kb_team_cfg.pass = pwd; _kb_team_cfg.m3u = m3u; _kb_team_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('KBC (Kinoboom)');
