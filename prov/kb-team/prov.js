version += " kb_team-0219";
p_pref = "kb-team";
parental = /XXX|Взрослые|Для взрослых|Эротика|18\+|Adults/i;
if (typeof stbGetItem === "function") {
    providerGetItem = function (e) {
        return stbGetItem(p_pref + e);
    };
    providerSetItem = function (e, r) {
        stbSetItem(p_pref + e, r);
    };
} else {
    providerGetItem = function (e) {
        return localStorage.getItem(p_pref + e);
    };
    providerSetItem = function (e, r) {
        localStorage.setItem(p_pref + e, r);
    };
}
providerDelItem = function (e) {
    return ottpStorage.del(p_pref + e);
};
providerHasItem = function (e) {
    return ottpStorage.has(p_pref + e);
};
providerHasItemValue = function (e) {
    return ottpStorage.hasValue(p_pref + e);
};
var _kb_team_cfg = { m3u: "", pass: "", server: "", user: "" };
function _kb_team_load() {
    try {
        var d = providerGetItem("cfg");
        if (d) _kb_team_cfg = JSON.parse(d);
    } catch (e) {}
    if (!(_kb_team_cfg.server || _kb_team_cfg.m3u))
        _kb_team_cfg = { m3u: "", pass: "", server: "", user: "" };
}
function _kb_team_save() {
    providerSetItem("cfg", JSON.stringify(_kb_team_cfg));
}
function getChannelPicon(e) {
    return chanels[e] ? chanels[e].logo || "" : "";
}
function getChannelUrl(e) {
    return chanels[e] ? chanels[e].url || "" : "";
}
function getEPGchanel(s, e) {
    e(s, null);
}
function addChan2cat(catName, hash) {
    if (!(catName && hash)) return;
    if (!cats[catName]) {
        catsArray.push(catName);
        cats[catName] = [];
    }
    cats[catName].push(hash);
}
function getChanelsArray(cb) {
    _kb_team_load();
    if (_kb_team_cfg.server && _kb_team_cfg.user && _kb_team_cfg.pass)
        _kb_team_xtream(cb);
    else if (_kb_team_cfg.m3u) _kb_team_m3u(cb);
    else {
        alert(_("Configure KBC (Kinoboom) in Settings -> Provider Settings"));
        cb();
    }
}
function _kb_team_m3u(cb) {
    $(launch_id).append(_("Loading M3U..."));
    $.ajax({
        error: function () {
            $.ajax({
                data: { url: "@" + _kb_team_cfg.m3u },
                dataType: "text",
                error: function () {
                    alert(_("Failed to load!"));
                    cb();
                },
                method: "post",
                success: function (d) {
                    _kb_team_parseM3U(d, cb);
                },
                timeout: 15e3,
                url: host + "/m3u/cp.php",
            });
        },
        success: function (d) {
            _kb_team_parseM3U(d, cb);
        },
        timeout: 15e3,
        url: _kb_team_cfg.m3u,
    });
}
function _kb_team_parseM3U(data, cb) {
    cList = [];
    chanels = {};
    cats = {};
    catsArray = [];
    try {
        var lines = data.split("#EXTINF:");
        var hdr = lines[0] || "";
        lines.shift();
        var lc = "";
        lines.forEach(function (b) {
            var p = b.split("\n");
            var inf = p[0] || "";
            var url = "";
            for (var i = 1; i < p.length; i++) {
                if (p[i].trim() && p[i].trim()[0] !== "#") {
                    url = p[i].trim();
                    break;
                }
            }
            if (!url) return;
            var name = "???";
            var ci = inf.indexOf(",");
            if (ci > 0) name = inf.substr(ci + 1).trim();
            var cat = "";
            var gm = inf.match(/group-title="([^"]*)"/i);
            if (gm) cat = gm[1];
            var logo = "";
            var lm = inf.match(/tvg-logo="([^"]*)"/i);
            if (lm) logo = lm[1];
            if (!cat) cat = lc || "Other";
            lc = cat;
            var h = xxHash32S(url, true);
            addChan2cat(cat, h);
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = {
                    ca: "",
                    caso: "",
                    category: { class: catsArray.indexOf(cat) + 2, name: cat },
                    channel_name: name,
                    epg: "",
                    logo: logo,
                    rec: 0,
                    time: 0,
                    time_to: 0,
                    tn: name,
                    url: url,
                };
            }
        });
    } catch (e) {
        console.error(e);
    }
    cb();
}
function _kb_team_xtream(cb) {
    $(launch_id).append(_("Loading from API..."));
    var api =
        _kb_team_cfg.server +
        "/player_api.php?username=" +
        encodeURIComponent(_kb_team_cfg.user) +
        "&password=" +
        encodeURIComponent(_kb_team_cfg.pass);
    $.ajax({ dataType: "json", timeout: 15e3, type: "GET", url: api })
        .done(function (r) {
            cList = [];
            chanels = {};
            cats = {};
            catsArray = [];
            if (!(r && r.live_streams)) {
                _kb_team_cfg.m3u =
                    api.replace("/player_api.php", "/get.php") +
                    "&type=m3u_plus&output=ts";
                _kb_team_m3u(cb);
                return;
            }
            var cm = {};
            if (r.categories)
                r.categories.forEach(function (c) {
                    cm[c.category_id] = c.category_name || "Unknown";
                });
            r.live_streams.forEach(function (s) {
                var h = xxHash32S(s.name, true);
                var cn = cm[s.category_id] || "Other";
                addChan2cat(cn, h);
                if (cList.indexOf(h) === -1) {
                    cList.push(h);
                    chanels[h] = {
                        ca: "",
                        caso: "",
                        category: {
                            class: catsArray.indexOf(cn) + 2,
                            name: cn,
                        },
                        channel_name: s.name,
                        epg: String(s.stream_id),
                        logo: s.stream_icon || "",
                        rec: 0,
                        time: 0,
                        time_to: 0,
                        tn: s.name,
                        url:
                            _kb_team_cfg.server +
                            "/live/" +
                            encodeURIComponent(_kb_team_cfg.user) +
                            "/" +
                            encodeURIComponent(_kb_team_cfg.pass) +
                            "/" +
                            s.stream_id +
                            ".m3u8",
                    };
                }
            });
            cb();
        })
        .fail(function () {
            _kb_team_cfg.m3u =
                _kb_team_cfg.server.replace(/\/+$/, "") +
                "/get.php?username=" +
                encodeURIComponent(_kb_team_cfg.user) +
                "&password=" +
                encodeURIComponent(_kb_team_cfg.pass) +
                "&type=m3u_plus&output=ts";
            _kb_team_m3u(cb);
        });
}
function duneAddSettings(e) {
    _kb_team_load();
    popupArray.splice(e, 1, "");
    popupDetail.splice(e, 1, _("KBC (Kinoboom) settings"));
    popupActions.splice(e, 1, _kb_team_edit);
    var idx = popupActions.indexOf(_kb_team_edit);
    if (idx > -1) {
        var lbl = _("KBC (Kinoboom) settings");
        if (_kb_team_cfg.server && _kb_team_cfg.user)
            lbl +=
                ": " +
                _kb_team_cfg.server.replace(/^https?:\/\//, "").split("/")[0] +
                " (" +
                _kb_team_cfg.user +
                ")";
        else if (_kb_team_cfg.m3u)
            lbl += ": " + _kb_team_cfg.m3u.substr(0, 40) + "...";
        popupArray[idx] = lbl;
    }
}
function _kb_team_edit() {
    selIndex = 0;
    _kb_team_load();
    var srv = _kb_team_cfg.server,
        usr = _kb_team_cfg.user,
        pwd = _kb_team_cfg.pass,
        m3u = _kb_team_cfg.m3u;
    function bl() {
        listArray = [
            _("Server") + ": " + (srv || ""),
            _("Login") + ": " + (usr || ""),
            _("Password") + ": " + (pwd ? "********" : ""),
            _("M3U") + ": " + (m3u ? m3u.substr(0, 45) : ""),
            "",
            _("Save and load"),
        ];
    }
    var ii = [
        _("API server URL"),
        _("Username"),
        _("Password"),
        _("M3U URL (fallback)"),
        "",
        _("Save & load channels"),
    ];
    bl();
    getListItem = function (e, r) {
        return "&nbsp;&nbsp;" + e;
    };
    detailListAction = function () {
        listDetail.innerHTML = ii[selIndex] || "";
    };
    listKeyHandler = function (e) {
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    case 0:
                        editCaption = _("Server URL");
                        editvar = srv;
                        setEdit = function () {
                            srv = editvar.trim();
                            bl();
                            showPage();
                        };
                        showEditKey(keys.ENTER);
                        return true;
                    case 1:
                        editCaption = _("Username");
                        editvar = usr;
                        setEdit = function () {
                            usr = editvar.trim();
                            bl();
                            showPage();
                        };
                        showEditKey(keys.ENTER);
                        return true;
                    case 2:
                        editCaption = _("Password");
                        editvar = pwd;
                        setEdit = function () {
                            pwd = editvar.trim();
                            bl();
                            showPage();
                        };
                        showEditKey(keys.ENTER);
                        return true;
                    case 3:
                        editCaption = _("M3U URL");
                        editvar = m3u;
                        setEdit = function () {
                            m3u = editvar.trim();
                            bl();
                            showPage();
                        };
                        showEditKey(keys.ENTER);
                        return true;
                    case 5:
                        _kb_team_cfg.server = srv;
                        _kb_team_cfg.user = usr;
                        _kb_team_cfg.pass = pwd;
                        _kb_team_cfg.m3u = m3u;
                        _kb_team_save();
                        duneAddSettings(0);
                        loadChannels();
                        return true;
                }
                return true;
            case keys.RETURN:
                popupList(popupActions.indexOf(noProvParam) + 1);
                return true;
            default:
                return false;
        }
    };
    listDetail.innerHTML = "";
    listCaption.innerHTML = _("KBC (Kinoboom)");
    listPodval.innerHTML = btnDiv(keys.RETURN, strRETURN, "Close");
    $("#listPopUp").hide();
    showPage();
}
