...
function _polmedia_load() {
    ...
    if (!(_polmedia_cfg.server || _polmedia_cfg.m3u))
        _polmedia_cfg = { server: "", user: "", pass: "", m3u: "" };
}
...
function getChanelsArray(cb) {
    _polmedia_load();
    if (_polmedia_cfg.server && _polmedia_cfg.user && _polmedia_cfg.pass)
        _polmedia_xtream(cb);
...
function _polmedia_m3u(cb) {
    ...
    $.ajax({
        url: _polmedia_cfg.m3u,
        timeout: 15e3,
        success: function (d) {
            _polmedia_parseM3U(d, cb);
        },
        error: function () {
            $.ajax({
                url: host + "/m3u/cp.php",
                data: { url: "@" + _polmedia_cfg.m3u },
                method: "post",
                dataType: "text",
                timeout: 15e3,
                success: function (d) {
...
function _polmedia_xtream(cb) {
    ...
    var api =
        _polmedia_cfg.server +
        "/player_api.php?username=" +
        encodeURIComponent(_polmedia_cfg.user) +
        "&password=" +
        encodeURIComponent(_polmedia_cfg.pass);
