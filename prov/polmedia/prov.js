...
var _polmedia_cfg = { server: '', user: '', pass: '', m3u: '' };
function _polmedia_load() {
    try { var d = providerGetItem('cfg'); if (d) _polmedia_cfg = JSON.parse(d); } catch(e) {}
    if (!_polmedia_cfg.server && !_polmedia_cfg.m3u) _polmedia_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _polmedia_save() { providerSetItem('cfg', JSON.stringify(_polmedia_cfg)); }
...
function getChanelsArray(cb) {
    _polmedia_load();
    if (_polmedia_cfg.server && _polmedia_cfg.user && _polmedia_cfg.pass) _polmedia_xtream(cb);
    else if (_polmedia_cfg.m3u) _polmedia_m3u(cb);
    else { alert(_('Configure POLMEDIA in Settings -> Provider Settings')); cb(); }
}
function _polmedia_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _polmedia_cfg.m3u, timeout: 15e3,
        success: function(d) { _polmedia_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _polmedia_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _polmedia_parseM3U(d, cb); },
...
function _polmedia_xtream(cb) {
    ...
    var api = _polmedia_cfg.server + '/player_api.php?username=' + encodeURIComponent(_polmedia_cfg.user) + '&password=' + encodeURIComponent(_polmedia_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _polmedia_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _polmedia_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _polmedia_cfg.server + '/live/' + encodeURIComponent(_polmedia_cfg.user) + '/' + encodeURIComponent(_polmedia_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _polmedia_cfg.m3u = _polmedia_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_polmedia_cfg.user) + '&password=' + encodeURIComponent(_polmedia_cfg.pass) + '&type=m3u_plus&output=ts';
        _polmedia_m3u(cb);
    });
}
function duneAddSettings(e) {
    _polmedia_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('POLMEDIA settings'));
    popupActions.splice(e, 1, _polmedia_edit);
    var idx = popupActions.indexOf(_polmedia_edit);
    if (idx > -1) {
        var lbl = _('POLMEDIA settings');
        if (_polmedia_cfg.server && _polmedia_cfg.user) lbl += ': ' + _polmedia_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _polmedia_cfg.user + ')';
        else if (_polmedia_cfg.m3u) lbl += ': ' + _polmedia_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _polmedia_edit() {
    selIndex = 0; _polmedia_load();
    var srv = _polmedia_cfg.server, usr = _polmedia_cfg.user, pwd = _polmedia_cfg.pass, m3u = _polmedia_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _polmedia_cfg.server = srv; _polmedia_cfg.user = usr; _polmedia_cfg.pass = pwd; _polmedia_cfg.m3u = m3u; _polmedia_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('POLMEDIA');
