...
var _great_cfg = { server: '', user: '', pass: '', m3u: '' };
function _great_load() {
    try { var d = providerGetItem('cfg'); if (d) _great_cfg = JSON.parse(d); } catch(e) {}
    if (!_great_cfg.server && !_great_cfg.m3u) _great_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _great_save() { providerSetItem('cfg', JSON.stringify(_great_cfg)); }
...
function getChanelsArray(cb) {
    _great_load();
    if (_great_cfg.server && _great_cfg.user && _great_cfg.pass) _great_xtream(cb);
    else if (_great_cfg.m3u) _great_m3u(cb);
    else { alert(_('Configure GREAT IPTV in Settings -> Provider Settings')); cb(); }
}
function _great_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _great_cfg.m3u, timeout: 15e3,
        success: function(d) { _great_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _great_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _great_parseM3U(d, cb); },
...
function _great_xtream(cb) {
    ...
    var api = _great_cfg.server + '/player_api.php?username=' + encodeURIComponent(_great_cfg.user) + '&password=' + encodeURIComponent(_great_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _great_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _great_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _great_cfg.server + '/live/' + encodeURIComponent(_great_cfg.user) + '/' + encodeURIComponent(_great_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _great_cfg.m3u = _great_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_great_cfg.user) + '&password=' + encodeURIComponent(_great_cfg.pass) + '&type=m3u_plus&output=ts';
        _great_m3u(cb);
    });
}
function duneAddSettings(e) {
    _great_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('GREAT IPTV settings'));
    popupActions.splice(e, 1, _great_edit);
    var idx = popupActions.indexOf(_great_edit);
    if (idx > -1) {
        var lbl = _('GREAT IPTV settings');
        if (_great_cfg.server && _great_cfg.user) lbl += ': ' + _great_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _great_cfg.user + ')';
        else if (_great_cfg.m3u) lbl += ': ' + _great_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _great_edit() {
    selIndex = 0; _great_load();
    var srv = _great_cfg.server, usr = _great_cfg.user, pwd = _great_cfg.pass, m3u = _great_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _great_cfg.server = srv; _great_cfg.user = usr; _great_cfg.pass = pwd; _great_cfg.m3u = m3u; _great_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('GREAT IPTV');
