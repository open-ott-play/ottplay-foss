...
function _great_load() {
    ...
    if (!(_great_cfg.server || _great_cfg.m3u))
        _great_cfg = { server: "", user: "", pass: "", m3u: "" };
}
...
function getChanelsArray(cb) {
    _great_load();
    if (_great_cfg.server && _great_cfg.user && _great_cfg.pass)
        _great_xtream(cb);
...
function _great_m3u(cb) {
    ...
    $.ajax({
        ...
        timeout: 15e3,
        success: function (d) {
            _great_parseM3U(d, cb);
        },
        error: function () {
            $.ajax({
                url: host + "/m3u/cp.php",
                data: { url: "@" + _great_cfg.m3u },
                method: "post",
                dataType: "text",
                timeout: 15e3,
                success: function (d) {
...
function _great_xtream(cb) {
    ...
    var api =
        ...
        encodeURIComponent(_great_cfg.user) +
        "&password=" +
        encodeURIComponent(_great_cfg.pass);
