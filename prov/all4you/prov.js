...
function _all4you_load() {
    try { var d = providerGetItem('cfg'); if (d) _all4you_cfg = JSON.parse(d); } catch(e) {}
    if (!_all4you_cfg.server && !_all4you_cfg.m3u) _all4you_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _all4you_save() { providerSetItem('cfg', JSON.stringify(_all4you_cfg)); }
...
function getChanelsArray(cb) {
    _all4you_load();
    if (_all4you_cfg.server && _all4you_cfg.user && _all4you_cfg.pass) _all4you_xtream(cb);
    else if (_all4you_cfg.m3u) _all4you_m3u(cb);
    else { alert(_('Configure All4you.tv in Settings -> Provider Settings')); cb(); }
}
function _all4you_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _all4you_cfg.m3u, timeout: 15e3,
        success: function(d) { _all4you_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _all4you_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _all4you_parseM3U(d, cb); },
...
function _all4you_xtream(cb) {
    ...
    var api = _all4you_cfg.server + '/player_api.php?username=' + encodeURIComponent(_all4you_cfg.user) + '&password=' + encodeURIComponent(_all4you_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _all4you_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _all4you_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _all4you_cfg.server + '/live/' + encodeURIComponent(_all4you_cfg.user) + '/' + encodeURIComponent(_all4you_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _all4you_cfg.m3u = _all4you_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_all4you_cfg.user) + '&password=' + encodeURIComponent(_all4you_cfg.pass) + '&type=m3u_plus&output=ts';
        _all4you_m3u(cb);
    });
}
function duneAddSettings(e) {
    _all4you_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('All4you.tv settings'));
    popupActions.splice(e, 1, _all4you_edit);
    var idx = popupActions.indexOf(_all4you_edit);
    if (idx > -1) {
        var lbl = _('All4you.tv settings');
        if (_all4you_cfg.server && _all4you_cfg.user) lbl += ': ' + _all4you_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _all4you_cfg.user + ')';
        else if (_all4you_cfg.m3u) lbl += ': ' + _all4you_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _all4you_edit() {
    selIndex = 0; _all4you_load();
    var srv = _all4you_cfg.server, usr = _all4you_cfg.user, pwd = _all4you_cfg.pass, m3u = _all4you_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _all4you_cfg.server = srv; _all4you_cfg.user = usr; _all4you_cfg.pass = pwd; _all4you_cfg.m3u = m3u; _all4you_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('All4you.tv');
