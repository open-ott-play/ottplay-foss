...
var _antifriz_cfg = { server: '', user: '', pass: '', m3u: '' };
function _antifriz_load() {
    try { var d = providerGetItem('cfg'); if (d) _antifriz_cfg = JSON.parse(d); } catch(e) {}
    if (!_antifriz_cfg.server && !_antifriz_cfg.m3u) _antifriz_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _antifriz_save() { providerSetItem('cfg', JSON.stringify(_antifriz_cfg)); }
...
function getChanelsArray(cb) {
    _antifriz_load();
    if (_antifriz_cfg.server && _antifriz_cfg.user && _antifriz_cfg.pass) _antifriz_xtream(cb);
    else if (_antifriz_cfg.m3u) _antifriz_m3u(cb);
    else { alert(_('Configure АнтиФриз.ТВ in Settings -> Provider Settings')); cb(); }
}
function _antifriz_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _antifriz_cfg.m3u, timeout: 15e3,
        success: function(d) { _antifriz_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _antifriz_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _antifriz_parseM3U(d, cb); },
...
function _antifriz_xtream(cb) {
    ...
    var api = _antifriz_cfg.server + '/player_api.php?username=' + encodeURIComponent(_antifriz_cfg.user) + '&password=' + encodeURIComponent(_antifriz_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _antifriz_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _antifriz_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _antifriz_cfg.server + '/live/' + encodeURIComponent(_antifriz_cfg.user) + '/' + encodeURIComponent(_antifriz_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _antifriz_cfg.m3u = _antifriz_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_antifriz_cfg.user) + '&password=' + encodeURIComponent(_antifriz_cfg.pass) + '&type=m3u_plus&output=ts';
        _antifriz_m3u(cb);
    });
}
function duneAddSettings(e) {
    _antifriz_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('АнтиФриз.ТВ settings'));
    popupActions.splice(e, 1, _antifriz_edit);
    var idx = popupActions.indexOf(_antifriz_edit);
    if (idx > -1) {
        var lbl = _('АнтиФриз.ТВ settings');
        if (_antifriz_cfg.server && _antifriz_cfg.user) lbl += ': ' + _antifriz_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _antifriz_cfg.user + ')';
        else if (_antifriz_cfg.m3u) lbl += ': ' + _antifriz_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _antifriz_edit() {
    selIndex = 0; _antifriz_load();
    var srv = _antifriz_cfg.server, usr = _antifriz_cfg.user, pwd = _antifriz_cfg.pass, m3u = _antifriz_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _antifriz_cfg.server = srv; _antifriz_cfg.user = usr; _antifriz_cfg.pass = pwd; _antifriz_cfg.m3u = m3u; _antifriz_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('АнтиФриз.ТВ');
