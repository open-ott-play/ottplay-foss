...
var _tvteam_cfg = { server: '', user: '', pass: '', m3u: '' };
function _tvteam_load() {
    try { var d = providerGetItem('cfg'); if (d) _tvteam_cfg = JSON.parse(d); } catch(e) {}
    if (!_tvteam_cfg.server && !_tvteam_cfg.m3u) _tvteam_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _tvteam_save() { providerSetItem('cfg', JSON.stringify(_tvteam_cfg)); }
...
function getChanelsArray(cb) {
    _tvteam_load();
    if (_tvteam_cfg.server && _tvteam_cfg.user && _tvteam_cfg.pass) _tvteam_xtream(cb);
    else if (_tvteam_cfg.m3u) _tvteam_m3u(cb);
    else { alert(_('Configure TV.Team in Settings -> Provider Settings')); cb(); }
}
function _tvteam_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _tvteam_cfg.m3u, timeout: 15e3,
        success: function(d) { _tvteam_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _tvteam_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _tvteam_parseM3U(d, cb); },
...
function _tvteam_xtream(cb) {
    ...
    var api = _tvteam_cfg.server + '/player_api.php?username=' + encodeURIComponent(_tvteam_cfg.user) + '&password=' + encodeURIComponent(_tvteam_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _tvteam_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _tvteam_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _tvteam_cfg.server + '/live/' + encodeURIComponent(_tvteam_cfg.user) + '/' + encodeURIComponent(_tvteam_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _tvteam_cfg.m3u = _tvteam_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_tvteam_cfg.user) + '&password=' + encodeURIComponent(_tvteam_cfg.pass) + '&type=m3u_plus&output=ts';
        _tvteam_m3u(cb);
    });
}
function duneAddSettings(e) {
    _tvteam_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('TV.Team settings'));
    popupActions.splice(e, 1, _tvteam_edit);
    var idx = popupActions.indexOf(_tvteam_edit);
    if (idx > -1) {
        var lbl = _('TV.Team settings');
        if (_tvteam_cfg.server && _tvteam_cfg.user) lbl += ': ' + _tvteam_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _tvteam_cfg.user + ')';
        else if (_tvteam_cfg.m3u) lbl += ': ' + _tvteam_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _tvteam_edit() {
    selIndex = 0; _tvteam_load();
    var srv = _tvteam_cfg.server, usr = _tvteam_cfg.user, pwd = _tvteam_cfg.pass, m3u = _tvteam_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _tvteam_cfg.server = srv; _tvteam_cfg.user = usr; _tvteam_cfg.pass = pwd; _tvteam_cfg.m3u = m3u; _tvteam_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('TV.Team');
