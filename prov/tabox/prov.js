...
var _tabox_cfg = { server: '', user: '', pass: '', m3u: '' };
function _tabox_load() {
    try { var d = providerGetItem('cfg'); if (d) _tabox_cfg = JSON.parse(d); } catch(e) {}
    if (!_tabox_cfg.server && !_tabox_cfg.m3u) _tabox_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _tabox_save() { providerSetItem('cfg', JSON.stringify(_tabox_cfg)); }
...
function getChanelsArray(cb) {
    _tabox_load();
    if (_tabox_cfg.server && _tabox_cfg.user && _tabox_cfg.pass) _tabox_xtream(cb);
    else if (_tabox_cfg.m3u) _tabox_m3u(cb);
    else { alert(_('Configure Tabox in Settings -> Provider Settings')); cb(); }
}
function _tabox_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _tabox_cfg.m3u, timeout: 15e3,
        success: function(d) { _tabox_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _tabox_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _tabox_parseM3U(d, cb); },
...
function _tabox_parseM3U(data, cb) {
...
function _tabox_xtream(cb) {
    $(launch_id).append(_('Loading from API...'));
    var api = _tabox_cfg.server + '/player_api.php?username=' + encodeURIComponent(_tabox_cfg.user) + '&password=' + encodeURIComponent(_tabox_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _tabox_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _tabox_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _tabox_cfg.server + '/live/' + encodeURIComponent(_tabox_cfg.user) + '/' + encodeURIComponent(_tabox_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _tabox_cfg.m3u = _tabox_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_tabox_cfg.user) + '&password=' + encodeURIComponent(_tabox_cfg.pass) + '&type=m3u_plus&output=ts';
        _tabox_m3u(cb);
    });
}
function duneAddSettings(e) {
    _tabox_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('Tabox settings'));
    popupActions.splice(e, 1, _tabox_edit);
    var idx = popupActions.indexOf(_tabox_edit);
    if (idx > -1) {
        var lbl = _('Tabox settings');
        if (_tabox_cfg.server && _tabox_cfg.user) lbl += ': ' + _tabox_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _tabox_cfg.user + ')';
        else if (_tabox_cfg.m3u) lbl += ': ' + _tabox_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _tabox_edit() {
    selIndex = 0; _tabox_load();
    var srv = _tabox_cfg.server, usr = _tabox_cfg.user, pwd = _tabox_cfg.pass, m3u = _tabox_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _tabox_cfg.server = srv; _tabox_cfg.user = usr; _tabox_cfg.pass = pwd; _tabox_cfg.m3u = m3u; _tabox_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('Tabox');
