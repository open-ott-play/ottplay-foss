...
function _tabox_load() {
    try {
        ...
        if (d) _tabox_cfg = JSON.parse(d);
    } catch (e) {}
    if (!(_tabox_cfg.server || _tabox_cfg.m3u))
        _tabox_cfg = { server: "", user: "", pass: "", m3u: "" };
}
function _tabox_save() {
    providerSetItem("cfg", JSON.stringify(_tabox_cfg));
}
...
function getChanelsArray(cb) {
    _tabox_load();
    if (_tabox_cfg.server && _tabox_cfg.user && _tabox_cfg.pass)
        _tabox_xtream(cb);
    else if (_tabox_cfg.m3u) _tabox_m3u(cb);
    else {
        alert(_("Configure Tabox in Settings -> Provider Settings"));
        cb();
    }
}
function _tabox_m3u(cb) {
    ...
    $.ajax({
        ...
        timeout: 15e3,
        success: function (d) {
            _tabox_parseM3U(d, cb);
        },
        error: function () {
            $.ajax({
                url: host + "/m3u/cp.php",
                data: { url: "@" + _tabox_cfg.m3u },
                method: "post",
                dataType: "text",
                timeout: 15e3,
                success: function (d) {
...
function _tabox_xtream(cb) {
    ...
    var api =
        ...
        encodeURIComponent(_tabox_cfg.user) +
        "&password=" +
        encodeURIComponent(_tabox_cfg.pass);
...
function duneAddSettings(e) {
    ...
    popupDetail.splice(e, 1, _("Tabox settings"));
    popupActions.splice(e, 1, _tabox_edit);
    var idx = popupActions.indexOf(_tabox_edit);
    if (idx > -1) {
        var lbl = _("Tabox settings");
        if (_tabox_cfg.server && _tabox_cfg.user)
            lbl +=
                ": " +
                _tabox_cfg.server.replace(/^https?:\/\//, "").split("/")[0] +
                " (" +
                _tabox_cfg.user +
                ")";
        else if (_tabox_cfg.m3u)
            lbl += ": " + _tabox_cfg.m3u.substr(0, 40) + "...";
        popupArray[idx] = lbl;
    }
}
