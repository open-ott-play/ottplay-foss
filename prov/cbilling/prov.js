...
var _cbilling_cfg = { server: '', user: '', pass: '', m3u: '' };
function _cbilling_load() {
    try { var d = providerGetItem('cfg'); if (d) _cbilling_cfg = JSON.parse(d); } catch(e) {}
    if (!_cbilling_cfg.server && !_cbilling_cfg.m3u) _cbilling_cfg = { server: '', user: '', pass: '', m3u: '' };
}
function _cbilling_save() { providerSetItem('cfg', JSON.stringify(_cbilling_cfg)); }
...
function getChanelsArray(cb) {
    _cbilling_load();
    if (_cbilling_cfg.server && _cbilling_cfg.user && _cbilling_cfg.pass) _cbilling_xtream(cb);
    else if (_cbilling_cfg.m3u) _cbilling_m3u(cb);
    else { alert(_('Configure Гомельсат (cbilling) in Settings -> Provider Settings')); cb(); }
}
function _cbilling_m3u(cb) {
    $(launch_id).append(_('Loading M3U...'));
    $.ajax({ url: _cbilling_cfg.m3u, timeout: 15e3,
        success: function(d) { _cbilling_parseM3U(d, cb); },
        error: function() {
            $.ajax({ url: host + '/m3u/cp.php', data: { url: '@' + _cbilling_cfg.m3u }, method: 'post', dataType: 'text', timeout: 15e3,
                success: function(d) { _cbilling_parseM3U(d, cb); },
...
function _cbilling_xtream(cb) {
    ...
    var api = _cbilling_cfg.server + '/player_api.php?username=' + encodeURIComponent(_cbilling_cfg.user) + '&password=' + encodeURIComponent(_cbilling_cfg.pass);
    $.ajax({ type: 'GET', url: api, dataType: 'json', timeout: 15e3
    }).done(function(r) {
        cList = []; chanels = {}; cats = {}; catsArray = [];
        if (!r || !r.live_streams) { _cbilling_cfg.m3u = api.replace('/player_api.php', '/get.php') + '&type=m3u_plus&output=ts'; _cbilling_m3u(cb); return; }
        ...
        r.live_streams.forEach(function(s) {
            ...
            if (cList.indexOf(h) === -1) {
                cList.push(h);
                chanels[h] = { channel_name: s.name, category: { class: catsArray.indexOf(cn) + 2, name: cn }, rec: 0, time: 0, time_to: 0, url: _cbilling_cfg.server + '/live/' + encodeURIComponent(_cbilling_cfg.user) + '/' + encodeURIComponent(_cbilling_cfg.pass) + '/' + s.stream_id + '.m3u8', logo: s.stream_icon || '', epg: String(s.stream_id), tn: s.name, ca: '', caso: '' };
            }
        });
        cb();
    }).fail(function() {
        _cbilling_cfg.m3u = _cbilling_cfg.server.replace(/\/+$/, '') + '/get.php?username=' + encodeURIComponent(_cbilling_cfg.user) + '&password=' + encodeURIComponent(_cbilling_cfg.pass) + '&type=m3u_plus&output=ts';
        _cbilling_m3u(cb);
    });
}
function duneAddSettings(e) {
    _cbilling_load();
    popupArray.splice(e, 1, ''); popupDetail.splice(e, 1, _('Гомельсат (cbilling) settings'));
    popupActions.splice(e, 1, _cbilling_edit);
    var idx = popupActions.indexOf(_cbilling_edit);
    if (idx > -1) {
        var lbl = _('Гомельсат (cbilling) settings');
        if (_cbilling_cfg.server && _cbilling_cfg.user) lbl += ': ' + _cbilling_cfg.server.replace(/^https?:\/\//, '').split('/')[0] + ' (' + _cbilling_cfg.user + ')';
        else if (_cbilling_cfg.m3u) lbl += ': ' + _cbilling_cfg.m3u.substr(0, 40) + '...';
        popupArray[idx] = lbl;
    }
}
function _cbilling_edit() {
    selIndex = 0; _cbilling_load();
    var srv = _cbilling_cfg.server, usr = _cbilling_cfg.user, pwd = _cbilling_cfg.pass, m3u = _cbilling_cfg.m3u;
    function bl() {
        listArray = [
            _('Server') + ': ' + (srv || ''),
            _('Login') + ': ' + (usr || ''),
        ...
        switch (e) {
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 5: _cbilling_cfg.server = srv; _cbilling_cfg.user = usr; _cbilling_cfg.pass = pwd; _cbilling_cfg.m3u = m3u; _cbilling_save(); duneAddSettings(0); loadChannels(); return true;
                }
                return true;
    ...
    listDetail.innerHTML = ''; listCaption.innerHTML = _('Гомельсат (cbilling)');
