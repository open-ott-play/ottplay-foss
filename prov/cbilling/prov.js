...
function _cbilling_load() {
    try {
        ...
        if (d) _cbilling_cfg = JSON.parse(d);
    } catch (e) {}
    if (!(_cbilling_cfg.server || _cbilling_cfg.m3u))
        _cbilling_cfg = { server: "", user: "", pass: "", m3u: "" };
}
function _cbilling_save() {
    providerSetItem("cfg", JSON.stringify(_cbilling_cfg));
}
...
function duneAddSettings(e) {
    ...
    popupDetail.splice(e, 1, _("Гомельсат (cbilling) settings"));
    popupActions.splice(e, 1, _cbilling_edit);
    var idx = popupActions.indexOf(_cbilling_edit);
    if (idx > -1) {
        var lbl = _("Гомельсат (cbilling) settings");
        if (_cbilling_cfg.server && _cbilling_cfg.user)
            lbl +=
                ": " +
                _cbilling_cfg.server.replace(/^https?:\/\//, "").split("/")[0] +
                " (" +
                _cbilling_cfg.user +
                ")";
        else if (_cbilling_cfg.m3u)
            lbl += ": " + _cbilling_cfg.m3u.substr(0, 40) + "...";
        popupArray[idx] = lbl;
    }
}
function _cbilling_edit() {
            ...
            case keys.ENTER:
                switch (selIndex) {
                    ...
                    case 3:
                        ...
                        return true;
                    case 5:
                        _cbilling_cfg.server = srv;
                        _cbilling_cfg.user = usr;
                        _cbilling_cfg.pass = pwd;
                        _cbilling_cfg.m3u = m3u;
                        _cbilling_save();
                        duneAddSettings(0);
                        loadChannels();
                        return true;
    ...
    listCaption.innerHTML = _("Гомельсат (cbilling)");
