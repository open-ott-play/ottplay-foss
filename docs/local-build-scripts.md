...
Same idea as `inverter-desktop`’s `build-*-local.sh`, adapted for OttPlay FOSS (**Tauri** desktop + **Capacitor** iOS).
...
Common flags: `--clean`, `--update-deps`. iOS also supports `--dev`. iOS sim: `--device 'iPhone 16'` or `IOS_SIM_DEVICE=…`.
